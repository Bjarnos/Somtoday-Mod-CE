...
async function build() {
    ...
    const ignoredInCopy = ["manifest.chromium.json", "manifest.firefox.json"];
    ...
    const chromiumVersionInfo = {
        ...versionInfo,
        platform: "Chromium",
        minified: false,
    };
    fs.writeFileSync(
        path.join(chromiumDir, "version_info.json"),
        JSON.stringify(chromiumVersionInfo, null, 4),
    );
    ...
    const firefoxVersionInfo = {
        ...versionInfo,
        platform: "Firefox",
        minified: false,
    };
    fs.writeFileSync(
        path.join(firefoxDir, "version_info.json"),
        JSON.stringify(firefoxVersionInfo, null, 4),
    );
    ...
    const accessibleResources =
        chromiumManifest.web_accessible_resources[0].resources;
    ...
    fileMap["version_info.json"] =
        `data:application/json;base64,${Buffer.from(JSON.stringify(userscriptVersionInfo)).toString("base64")}`;
    ...
    const matches = chromiumManifest.host_permissions
        .map((h) => `// @match        ${h}`)
        .join("\n");
    const currentYear = new Date().getFullYear();

    const getHeader = (isMinified) => {
        const scriptFileName = isMinified
            ? "SomtodayMod.min.user.js"
            : "SomtodayMod.user.js";
...
`;
    ...
    minifiedFileMap["version_info.json"] =
        `data:application/json;base64,${Buffer.from(JSON.stringify(minifiedVersionInfo)).toString("base64")}`;
    ...
    const minifiedResult = await esbuild.transform(
        userscriptWithMinifiedFileMap,
        {
            minify: true,
