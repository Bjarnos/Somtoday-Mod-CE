...
document.getElementById("version").innerHTML =
    chrome.i18n.getMessage("extVersion");
document.getElementById("activetext").innerHTML =
    chrome.i18n.getMessage("activeText");
...
document.getElementById("whats-new").innerHTML =
    chrome.i18n.getMessage("whatsNew");
document.getElementById("new-header").innerHTML =
    chrome.i18n.getMessage("whatsNewHeader");
...
document.getElementById("whats-new").addEventListener("click", () => {
    ...
    document.getElementById("new-in-this-release").style.display = "block";
});
document.getElementById("back").addEventListener("click", () => {
    document.getElementById("activated").style.display = "block";
    document.getElementById("new-in-this-release").style.display = "none";
});
// Color functions
const hexToRgb = (hex) =>
    hex
...
chrome.storage.local.get(["primarycolor", "fontname"]).then((result) => {
    ...
    if (rgbcolor[2] > rgbcolor[0] && rgbcolor[2] > rgbcolor[1]) {
        const factor = -Math.min(
            Math.round(
                (rgbcolor[2] - rgbcolor[0] + (rgbcolor[2] - rgbcolor[1])) / 10,
            ),
            25,
        );
        background = adjustColorChannel(
            1,
            adjustColorChannel(0, toBrightnessValue(color, 245), factor),
            factor,
        );
        bordercolor = adjustColorChannel(
            1,
            adjustColorChannel(0, toBrightnessValue(color, 220), factor),
            factor,
        );
        borderhover = adjustColorChannel(
            1,
            adjustColorChannel(0, toBrightnessValue(color, 200), factor),
            factor,
        );
    }
    // Color is mostly red
        ...
        const factor = -Math.min(
            Math.round(
                (rgbcolor[0] - rgbcolor[2] + (rgbcolor[0] - rgbcolor[1])) / 10,
            ),
            25,
        );
        background = adjustColorChannel(
            2,
            adjustColorChannel(1, toBrightnessValue(color, 245), factor),
            factor,
        );
        bordercolor = adjustColorChannel(
            2,
            adjustColorChannel(1, toBrightnessValue(color, 220), factor),
            factor,
        );
        borderhover = adjustColorChannel(
            2,
            adjustColorChannel(1, toBrightnessValue(color, 200), factor),
            factor,
        );
    }
    // Color is mostly green
        ...
        const factor = -Math.min(
            Math.round(
                (rgbcolor[1] - rgbcolor[0] + (rgbcolor[1] - rgbcolor[2])) / 10,
            ),
            25,
        );
        background = adjustColorChannel(
