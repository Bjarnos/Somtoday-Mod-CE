...
function onload() {
        ...
        setTimeout(
            console.warn.bind(
                ...
                "SOMTODAY MOD:\nMultiple instances of Somtoday Mod are running.\nSomtoday Mod " +
                    ...
                    " v" +
                    version +
                    " will not be working until the other instance is deleted or deactivated.",
            ),
        );
        return;
    }
    ...
    const mathQuestions = [
        "compose the formula of f'x() where f(x) = 2 * sin(9x + 3)",
        "rewrite 2 * log3(243) + 28 to the form of log(a)",
        "find the value(s) of x in the equation x^2 + 9 = -6x",
        "give the point(s) where  12 = 2y + 6x and y = x^2 - 5x intersect (round on 2 decimals)",
        "rewrite -sin(8x) to the form of cos(ax + b)",
    ];
    const mathAnswers = [
        "Answer: f'(x) = 18 * cos(9x + 3)",
        "Answer: log(10^38)",
        "Answer: x = -3",
        "Answer: (-1,65; 10,94), (3,65; -4,94)",
        "Answer: cos(8x + 0.5pi)",
    ];
    const selectedQuestion = Math.floor(Math.random() * mathQuestions.length);
    tn("body", 0).insertAdjacentHTML(
        "beforeend",
        '<div id="somtoday-mod"><div id="somtoday-mod-active" data-platform="' +
            platform +
            '" data-version="' +
            version +
            '"><!-- Well hello there! Great work, detective. --><!-- Nothing better to do? Solve this math question: ' +
            mathQuestions[selectedQuestion] +
            ' --><div data-info="expand-to-view-answer"><!-- ' +
            mathAnswers[selectedQuestion] +
            " --></div></div></div>",
    );
    ...
    if (cn("cf-error-details cf-error-502", 0)) {
        setTimeout(
            console.warn.bind(console, "SOMTODAY MOD: Bad gateway (502)"),
        );
        return;
    }
    ...
    if (cn("titlewrap", 0)) {
        ...
        if (
            cn("titlewrap", 0).parentElement.parentElement.innerHTML.indexOf(
                "storing",
            ) != -1
        ) {
            setTimeout(
    ...
    let filesProcessed;
    let darkmode =
        tn("html", 0).classList.contains("dark") ||
        tn("html", 0).classList.contains("night");
    let busy = false;
    ...
    function consoleMessage() {
        setTimeout(
            console.log.bind(
                console,
                "%cSomtoday Mod is geactiveerd!",
                "color:#0067c2;font-weight:bold;font-family:Arial;font-size:26px;",
            ),
        );
        setTimeout(
            console.log.bind(
                console,
                "%cGeniet van je betere versie van Somtoday.\n\nMet dank aan " +
                    Object.keys(contributors).join(", ") +
                    "\nVersie " +
                    version_name +
                    " van Somtoday Mod " +
                    platform,
                "color:#0067c2;font-weight:bold;font-family:Arial;font-size:16px;",
            ),
        );
    }
    ...
    function initTheme() {
        ...
        if (
            (theme == "light" || theme == "dark" || theme == "night") &&
            get("autotheme") !== "true"
        ) {
    ...
    function easterEggs() {
        if (n(id("mod-easter-eggs"))) {
            ...
            document.addEventListener("keydown", (e) => {
                ...
                // Somtoday Mod
                const somtodaymodkeys = "somtoday mod shake";
                ...
                // Barrel roll
                const barrelrollkeys = "do a barrel roll";
                ...
                // Barrel roll x times
                const barrelrollxkeys = "do a barrel roll x";
                ...
                if (l == barrelrollxkeys.length) {
                    modMessage(
                        "BARREL ROLL",
                        'Hoeveel barrel rolls?<div class="br"></div><input id="mod-barrel-roll-amount" type="number" min="0" step="1" onkeyup="if (this.value != \'\') { this.value = Math.floor(this.value); } if (this.value < 1 && this.value != \'\') { this.value = 1; }"/>',
                        "Doorgaan",
                        "Annuleren",
                    );
                    id("mod-message-action1").addEventListener("click", () => {
                        closeModMessage();
                        tn("html", 0).classList.add("barrelroll");
                        setTimeout(
                            () => {
                    ...
                    id("mod-message-action2").addEventListener(
                        "click",
                        closeModMessage,
                    );
                }
                // Somtoday recap
                const recapkeys = "recap";
                ...
                if (p == partykeys.length) {
                    toggleConfetti();
                    try {
                        const tada = new Audio(window.getAudioUrl("tada"));
                        tada.volume = 0.5;
                        tada.play();
                    } catch (e) {
                        console.warn(e);
        ...
        if (
            id("somtoday-mod-version-easter-egg") &&
            !id("somtoday-mod-version-easter-egg").classList.contains(
                "mod-easter-egg",
            )
        ) {
            id("somtoday-mod-version-easter-egg").addEventListener(
                "click",
                () => {
                    tn("body", 0).insertAdjacentHTML(
                        "beforeend",
                        window.logo(
                            null,
                            'mod-easter-egg-logo mod-add-eventlistener" data-add-event-listener="true',
                            "#0099ff",
                        ),
                    );
                    for (const element of cn(
                        "mod-easter-egg-logo mod-add-eventlistener",
                    )) {
            ...
            id("somtoday-mod-version-easter-egg").classList.add(
                "mod-easter-egg",
            );
        }
        ...
        function easterEggShake() {
            var shakingElements = [];
            shakingElements = Array.prototype.concat.apply(
                shakingElements,
                cn("terug"),
            );
            shakingElements = Array.prototype.concat.apply(
            ...
            for (const element of shakingElements) {
                element.addEventListener("mouseover", function () {
                    shakeOnHover(this, -30, 30, -30, 30);
                });
            }
            for (const element of cn("afspraak-header")) {
                element.children[element.children.length - 1].addEventListener(
                    "mouseover",
                    function () {
                        shakeOnHover(this, -30, 30, -30, 30);
                    },
            ...
            tn("sl-header", 0).addEventListener("mouseover", function () {
                shakeOnHover(this, -20, 20, -20, 20);
            });
        }
        function shakeOnHover(element, xMin, xMax, yMin, yMax) {
            element.style.transform =
                "translate(" +
                (Math.random() * (xMax - xMin) + xMin).toString() +
                "px, " +
                (Math.random() * (yMax - yMin) + yMin).toString() +
                "px)";
        }
    }
    ...
    function customHomework() {
        ...
        if (
            get("bools").charAt(BOOL_INDEX.CUSTOM_HOMEWORK) == "1" &&
            !homeworkBusy
        ) {
            if (document.documentElement.clientWidth <= 767) {
                homeworkBusy = true;
                document
                    .querySelectorAll(".mod-huiswerk")
                    .forEach((e) => e.remove());
                document
                    .querySelectorAll(".mod-placeholder")
                    .forEach((e) => e.remove());
                homeworkBusy = false;
            }
            ...
            for (const element of tn("sl-studiewijzer-week")) {
                if (element.getElementsByClassName("week")[0]) {
                    index = customHomeworkInner(
                        element.getElementsByClassName("week")[0],
                        index,
                        startIndex,
                    );
                }
            ...
            if (tn("sl-rooster-week-header", 0)) {
                ...
                const elements = document.querySelectorAll(
                    "sl-rooster-week-header .mod-huiswerk",
                );
            ...
            document
                ...
                .forEach((e) => e.remove());
            document
                .querySelectorAll(".mod-add-homework")
                .forEach((e) => e.remove());
            document
                .querySelectorAll(".mod-placeholder")
                .forEach((e) => e.remove());
    ...
    function ariaLabelToDate(element) {
        ...
        if (element.getElementsByClassName("header")[0]) {
            ...
            dateString =
                (n(element.children[0].ariaLabel) ||
                !element.children[0].ariaLabel.match(/ (\d*? [a-z]*?)$/, "")
                    ? ""
                    : element.children[0].ariaLabel.match(
                          / (\d*? [a-z]*?)$/,
                          "",
                      )[1]) + " ";
        } else {
        ...
        if (
            dateString.indexOf("januari") != -1 ||
            dateString.indexOf("februari") != -1 ||
            dateString.indexOf("maart") != -1 ||
            dateString.indexOf("april") != -1 ||
            dateString.indexOf("mei") != -1 ||
            dateString.indexOf("juni") != -1 ||
            dateString.indexOf("juli") != -1
        ) {
        ...
        const englishDateString = dateString
    ...
    function customHomeworkIcons(activeIcon, activeColor) {
        ...
        for (const icon of icons) {
            iconHTML += window.getIcon(
                icon.name,
                "mod-homework-icon" +
                    (activeIcon == icon.name ? " mod-active" : ""),
                "var(" + icon.color + ")",
                'data-icon="' + icon.name + '" ',
            );
        }
        const col = window
            ...
            .getPropertyValue("--fg-warning-normal");
        return (
            '<div style="display:flex;margin-top:20px;align-items:center;gap:10px;flex-wrap:wrap;">' +
            iconHTML +
            '<label tabindex="0" for="homeworkcolor" style="margin-left:auto;cursor:pointer;">Kleur kiezen</label><input style="display:none;" value="' +
            (activeColor && activeColor.startsWith("#") ? activeColor : col) +
            '" id="homeworkcolor" type="color"></div>'
        );
    }
    ...
    function customHomeworkInner(element, startIndex, recurringTasksEndIndex) {
        ...
        const currentStudiewijzerDate = ariaLabelToDate(
        ...
        if (n(element.getElementsByClassName("mod-add-homework")[0])) {
            element.insertAdjacentHTML(
                "beforeend",
                '<div class="mod-add-homework">' +
                    window.getIcon("plus") +
                    "Taak toevoegen</div>",
            );
            element
                .getElementsByClassName("mod-add-homework")[0]
                .addEventListener("click", () => {
                    modMessage(
                        "Taak toevoegen",
                        "Voeg je eigen taak toe aan de kalender. De taak wordt alleen in deze browser opgeslagen.</p>" +
                            ...
                            '<div class="mod-multi-choice" id="studiewijzer-afspraak-toevoegen-select"><span class="active" tabindex="0">Eenmalig</span><span tabindex="0">Wekelijks</span>' +
                            (isWeek
                                ? ""
                                : '<span tabindex="0">Maandelijks</span>') +
                            "</div>" +
                    ...
                    id("mod-message-action1").addEventListener("click", () => {
                        const dateObject = ariaLabelToDate(
                            element.classList.contains("week")
                                ? element.nextElementSibling
                                : element,
                        );
                        homework.push({
                            ...
                            description: id("mod-homework-description").value,
                                ...
                                "studiewijzer-afspraak-toevoegen-select",
                            ...
                            returning: id(
                                ...
                                : id(
                                  ...
                                  : "2",
                            week: isWeek,
                            icon: cn("mod-homework-icon mod-active", 0).dataset
                                .icon,
                            color: cn(
                                "mod-homework-icon mod-active",
                                0,
                            ).children[0].getAttribute("fill"),
                        });
                        // Mostly sort by date
                        ...
                        for (let i = 0; i < homework.length; i++) {
                            if (
                                homework[i].returning == "0" &&
                                new Date().getFullYear() -
                                    new Date(
                                        Date.parse(homework[i].date),
                                    ).getFullYear() >=
                                    2
                            ) {
                                homework.splice(i, 1);
        ...
        if (!isNaN(currentStudiewijzerDate)) {
            function addHomework(j) {
                ...
                const homeworkDate = new Date(Date.parse(homework[i].date));
                const showIfOnce =
                    currentStudiewijzerDate.getFullYear() ==
                        homeworkDate.getFullYear() &&
                    currentStudiewijzerDate.getMonth() ==
                        homeworkDate.getMonth() &&
                    currentStudiewijzerDate.getDate() == homeworkDate.getDate();
                const showIfWeekly =
                    homework[i].returning == "1" &&
                    currentStudiewijzerDate.getTime() >=
                        homeworkDate.getTime() &&
                    currentStudiewijzerDate.getDay() == homeworkDate.getDay();
                const showIfMonthly =
                    homework[i].returning == "2" &&
                    currentStudiewijzerDate.getTime() >=
                        homeworkDate.getTime() &&
                    currentStudiewijzerDate.getDate() ==
                        homeworkDate.getDate() &&
                    !isWeek;
                if (
                    (showIfOnce || showIfWeekly || showIfMonthly) &&
                    n(
                        element.getElementsByClassName(
                            "mod-homework-" + homework[i].id,
                        )[0],
                    ) &&
                    ((isWeek && homework[i].week) ||
                        (!isWeek && !homework[i].week))
                ) {
                    lastShown = i;
                    let insertElement;
                    const studiewijzerItems = element.getElementsByTagName(
                        "sl-studiewijzer-items",
                    )[0];
                    if (
                        studiewijzerItems &&
                        !studiewijzerItems.classList.contains("mod-added") &&
                        studiewijzerItems.childElementCount == 0
                    ) {
                        ...
                        if (element.getElementsByClassName("header")[0]) {
                            element
                                ...
                                .insertAdjacentHTML(
                                    "afterend",
                                    '<sl-studiewijzer-items class="mod-added"></sl-studiewijzer-items>',
                                );
                        } else if (
                            element.getElementsByClassName("dag-header")[0]
                        ) {
                            element
                                .getElementsByClassName("dag-header")[0]
                                .insertAdjacentHTML(
                                    "afterend",
                                    '<sl-studiewijzer-items class="mod-added"></sl-studiewijzer-items>',
                                );
                            ...
                            element.insertAdjacentHTML(
                                "beforeend",
                                '<sl-studiewijzer-items class="mod-added mod-rooster"></sl-studiewijzer-items>',
                            );
                        }
                    }
                    let done;
                    ...
                    if (done) {
                        insertElement = element.getElementsByTagName(
                            "sl-studiewijzer-items",
                        )[0];
                    } else {
                        insertElement = element.getElementsByClassName(
                            "header",
                        )[0]
                            ? element.getElementsByClassName("header")[0]
                            : element.getElementsByClassName("dag-header")[0]
                              ? element.getElementsByClassName("dag-header")[0]
                    ...
                    if (!n(insertElement)) {
                        const noMoving =
                            ...
                            element
                                ...
                                .classList.contains("mod-rooster");
                        insertElement.insertAdjacentHTML(
                            "afterend",
                            '<div class="mod-huiswerk ' +
                                (done
                                    ? "mod-huiswerk-done"
                                    : noMoving
                                      ? ""
                                ...
                                "<strong>" +
                                sanitizeString(homework[i].subject) +
                                "</strong><p>" +
                                sanitizeString(homework[i].description) +
                                '</p><div><svg xmlns="http://www.w3.org/2000/svg" width="12px" height="12px" viewBox="0 0 24 24" display="block"><path fill-rule="evenodd" d="m9.706 21.576 13.876-14.05c.538-.55.56-1.43.044-1.998l-2.769-3.06a1.41 1.41 0 0 0-2.076-.025L9.83 11.858a1.41 1.41 0 0 1-2.06-.01L5.424 9.342a1.414 1.414 0 0 0-2.041-.032l-2.96 2.982a1.45 1.45 0 0 0 .003 2.052l7.27 7.242c.56.555 1.455.552 2.01-.01"></path></svg></div></div>',
                        );
                        const homeworkItem = insertElement.nextElementSibling;
                        const homeworkClassName =
                            "mod-homework-" + homework[i].id;
                        function saveAdjustedHomework(e) {
                            updateIndex();
                            e.stopPropagation();
                            homework[i].subject = id(
                                "mod-homework-subject",
                            ).value;
                            homework[i].description = id(
                                "mod-homework-description",
                            ).value;
                            homework[i].returning = id(
                                "studiewijzer-afspraak-toevoegen-select",
                            ).children[1].classList.contains("active")
                                ? "1"
                                : id(
                            ...
                            if (isRecurringAdjusted) {
                                homeworkBusy = true;
                                document
                                    .querySelectorAll("." + homeworkClassName)
                                    .forEach((e) => e.remove());
                                homeworkBusy = false;
                            }
                            closeModMessage();
                        }
                        homeworkItem.addEventListener("click", () => {
                            updateIndex();
                            modMessage(
                                "",
                                "</p>" +
                                    '<input id="mod-homework-subject" type="text" value="' +
                                    sanitizeString(homework[i].subject) +
                                    '" style="font-weight:700;font-size:20px;margin-top:-20px;"><div class="br"></div>' +
                                    "<textarea id=\"mod-homework-description\" oninput=\"document.getElementById('mod-message-action1').innerHTML = 'Opslaan';\">" +
                                    sanitizeString(homework[i].description) +
                                    "</textarea>" +
                                    '<div class="mod-multi-choice" id="studiewijzer-afspraak-toevoegen-select"><span' +
                                    (homework[i].returning == "0"
                                        ? ' class="active"'
                                        : "") +
                                    ' tabindex="0">Eenmalig</span><span' +
                                    (homework[i].returning == "1"
                                        ? ' class="active"'
                                        : "") +
                                    ' tabindex="0">Wekelijks</span>' +
                                    (isWeek
                                        ? ""
                                        : "<span" +
                                          (homework[i].returning == "2"
                            ...
                            id("homeworkcolor").addEventListener(
                                "input",
                                function () {
                                    for (const element of cn(
                                        "mod-homework-icon",
                            ...
                            id("mod-message-action1").addEventListener(
                                "click",
                                saveAdjustedHomework,
                            );
                            id("mod-message-action2").addEventListener(
                                "click",
                                (e) => {
                            ...
                            id("mod-message").addEventListener("click", () => {
                                closeModMessage();
                                tn("sl-root", 0).inert = false;
                                if (tn("sl-modal", 0)) {
                                    tn("sl-modal", 0).inert = false;
                        ...
                        homeworkItem
                            ...
                            .addEventListener("click", (e) => {
                                ...
                                if (
                                    homeworkItem.classList.contains(
                                        "mod-huiswerk-done",
                                    )
                                ) {
                                    homeworkItem.classList.remove(
                                        "mod-huiswerk-done",
                                    );
                                    homeworkItem.classList.remove(
                                        "mod-huiswerk-animation",
                                    );
                                    // Weekly/monthly task
                                    if (typeof homework[i].done === "object") {
                                        homework[i].done[
                                ...
                                // We always want to place the item as low as possible, since checked items always move to the bottom
                                    ...
                                    if (!noMoving) {
                                        let nextElement =
                                            homeworkItem.nextElementSibling;
                                        ...
                                        if (
                                            ...
                                            element
                                                ...
                                                .insertAdjacentHTML(
                                                    "beforebegin",
                                                    '<div class="mod-huiswerk mod-placeholder" style="visibility:hidden;"></div>',
                                                );
                                            homeworkItem.addEventListener(
                                                "animationend",
                                                () => {
                                                    if (
                                                        element.getElementsByClassName(
    ...
    function topMenu() {
        if (get("layout") == 5 && n(id("mod-top-menu"))) {
            tn("body", 0).insertAdjacentHTML(
                "beforeend",
                '<div id="mod-top-menu"><h2 id="mod-top-menu-title">Titel</h2><div id="mod-logout">' +
                    window.getIcon("right-from-bracket") +
                    '</div><div id="mod-messages">' +
                    window.getIcon("envelope") +
                    '</div><div id="mod-profile-link"></div></div>',
            );
            id("mod-profile-link").addEventListener("click", () => {
                cn("menu-avatar", 0).click();
            });
            id("mod-logout").addEventListener("click", () => {
                cn("menu-avatar", 0).click();
                tryRemove(id("mod-top-menu"));
                const checkLogoutButtonPresent = setInterval(() => {
            ...
            id("mod-messages").addEventListener("click", () => {
            ...
            cn("avatar", 0) &&
            cn("avatar", 0).getElementsByClassName("foto")[0] &&
            id("mod-profile-link")
        ) {
            id("mod-profile-link").innerHTML =
                "<div>" +
                (cn("avatar", 0)
                    .getElementsByClassName("foto")[0]
                    .classList.contains("hidden")
                    ? "<span>" +
                      (cn("avatar", 0).getElementsByClassName("initials")[0] &&
                      cn("avatar", 0).getElementsByClassName("initials")[0]
                          .children[0]
                          ? cn("avatar", 0).getElementsByClassName(
                                "initials",
        ...
        if (get("bools").charAt(BOOL_INDEX.GRADE_REVEAL) != "0") {
            if (tn("sl-laatsteresultaten", 0) && tn("sl-resultaat-item", 0)) {
                ...
                for (const element of cn("cijfer")) {
                    ...
                    if (!n(element.children[0])) {
                        if (
                            get("bools").charAt(BOOL_INDEX.GRADE_REVEAL) ==
                                "1" &&
                            element.children[0].innerHTML == lastGrade &&
                            (n(
                            ...
                            (n(
                                ...
                                element.parentElement.parentElement.parentElement
                                    .getElementsByClassName("subtitel")[0]
                                    .innerHTML.split("• ")[1] ==
                        ...
                        if (!isNaN(parseFloat(element.children[0].innerHTML))) {
                            countAnimation(
                                ...
                                0,
                                parseFloat(
                                    element.children[0].innerHTML.replace(
                                        ",",
                                        ".",
                                    ),
                                ),
    ...
    async function keyIsValid(key, value) {
        ...
        if (
            ...
            for (const nickname of json) {
                ...
                if (
                    /<\/?[a-z][\s\S]*>/i.test(nickname[0]) ||
                    /<\/?[a-z][\s\S]*>/i.test(nickname[1])
                ) {
            ...
            if (mayContainHTMLTag) {
                ...
                modMessage(
                    "Onveilige instellingswaarde",
                    'Het lijkt erop dat de instellingswaarde voor "Nicknames" HTML-elementen bevat, waarmee er onveilige code op je computer uitgevoerd kan worden. Wil je deze HTML elementen weghalen voor de veiligheid?',
                    "Weghalen",
                    "Behouden",
                    false,
                    true,
                ...
                id("mod-message-action2").addEventListener("click", () => {
                    set("nicknames", value);
                    closeModMessage();
                    saveReload(true);
                    tn("sl-root", 0).inert = false;
            ...
            if (/<\/?[a-z][\s\S]*>/i.test(value)) {
                ...
                modMessage(
                    "Onveilige instellingswaarde",
                    'Het lijkt erop dat de instellingswaarde voor "Username" HTML-elementen bevat, waarmee er onveilige code op je computer uitgevoerd kan worden. Wil je deze HTML elementen weghalen voor de veiligheid?',
                    "Weghalen",
                    "Behouden",
                    false,
                    true,
    ...
    async function importSettings() {
        execute([reset]);
        new Response(this.files[0]).json().then(
            (json) => {
                let closeModMessages = true;
                modMessage(
                    "Laden...",
                    "Dit kan even duren als je veel afbeeldingen hebt ingesteld...",
                );
            ...
            (err) => {
                modMessage(
                    "Oeps...",
                    "Het lijkt erop dat dit bestand ongeldig is.",
                    "Oke",
                );
                id("mod-message-action1").addEventListener(
                    "click",
    ...
    function newYearCountdown() {
        ...
        if (
            (ignoreCountdownConditions ||
                (get("bools").charAt(BOOL_INDEX.EVENTS) == "1" &&
                    Math.floor(diff / (1000 * 60 * 60 * 24)) <= 7)) &&
            !newYearCountdownClosed &&
            !id("mod-new-year")
        ) {
            ...
            if (rosterContainer) {
                ...
                setInterval(() => {
                    ...
                    h3.innerText = texts[current]
                        .replace(
                            "{percentage}",
                            (((yearDuration - diff) / yearDuration) * 100)
                                .toFixed(3)
                                .replace(".", ","),
                        )
                        .replace(
                            "{seconds_to_next_year}",
                ...
                rosterContainer.parentElement.insertAdjacentElement(
                    "beforebegin",
                    parent,
                );
                const fireworks = new Fireworks.default(
                    id("mod-new-year-fireworks"),
                    {
                        intensity: 12,
                        traceSpeed: 7,
                    },
                ...
                const p1 =
                    countdownElement.children[0].getElementsByTagName("p")[0];
                const p2 =
                    countdownElement.children[1].getElementsByTagName("p")[0];
                const p3 =
                    countdownElement.children[2].getElementsByTagName("p")[0];
                const p4 =
                    countdownElement.children[3].getElementsByTagName("p")[0];
                function updateCountdown() {
                    ...
                    if (p1.innerText !== days.toString()) {
                        ...
                        p1.innerText = days;
                        countdownElement.children[0].getElementsByTagName(
                            "span",
                        )[0].innerText = days == 1 ? "dag" : "dagen";
                        setTimeout(() => {
                    ...
                    if (p2.innerText !== hours.toString()) {
                        ...
                        p2.innerText = hours;
                        countdownElement.children[1].getElementsByTagName(
                            "span",
                        )[0].innerText = hours == 1 ? "uur" : "uren";
                        setTimeout(() => {
                    ...
                    if (p3.innerText !== minutes.toString()) {
                        ...
                        countdownElement.children[2].getElementsByTagName(
                            "span",
                        )[0].innerText = minutes == 1 ? "minuut" : "minuten";
                        setTimeout(() => {
                    ...
                    if (p4.innerText !== seconds.toString()) {
                        ...
                        countdownElement.children[3].getElementsByTagName(
                            "span",
                        )[0].innerText = seconds == 1 ? "seconde" : "seconden";
                        setTimeout(() => {
                    ...
                    if (
                        ...
                        rosterContainer.insertAdjacentElement(
                            "beforebegin",
                            fireworkElement,
                        );
                        const fireworks = new Fireworks.default(
    ...
    function rosterSimplify() {
        ...
        if (
            get("bools").charAt(BOOL_INDEX.ROSTER_SIMPLIFY) == "1" &&
            tn("sl-rooster-weken", 0)
        ) {
            if (
                cn("tertiary normal action-primary-normal center", 0) &&
                !cn(
                    "tertiary normal action-primary-normal center",
                    0,
                ).classList.contains("mod-vandaag-button-event-listener-added")
            ) {
                ...
                cn(
                    "tertiary normal action-primary-normal center",
                    0,
                ).classList.add("mod-vandaag-button-event-listener-added");
            }
            tn("sl-rooster-weken", 0).style.minHeight = "calc(100vh - 175px)";
            // Get the height the roster takes up normally
            normalRosterHeight = Math.max(
                tn("sl-rooster-weken", 0).clientHeight,
                normalRosterHeight,
            );
            ...
            for (const parent of tn("sl-rooster-week")) {
                ...
                for (const element of parent.getElementsByTagName(
                    ...
                    if (
                        n(element.getElementsByClassName("opacity-80")[0]) ||
                        element.getElementsByClassName("opacity-80")[0]
                            .innerHTML.length == 0 ||
                        isNaN(
                            parseInt(
                                element
                                    .getElementsByClassName("opacity-80")[0]
                                    .innerHTML.charAt(0),
                                10,
                            ),
                        )
                        ...
                        if (isWeekShown) {
                            tn("sl-rooster-weken", 0).style.height =
                                normalRosterHeight + "px";
                            tn("sl-rooster-weken", 0).style.overflowY =
                                "visible";
                            // Update the time text at the left to display 'hh:mm' instead of 'nth hour'
                            if (tn("sl-rooster-tijden", 0)) {
                                let i = 0;
                                for (const element of tn("sl-rooster-tijden", 0)
                                    .children) {
                                    const span =
                                        element.getElementsByTagName("span")[0];
                                    if (
                                        !n(span) &&
                                        !n(span.dataset.modOriginalContent)
                                    ) {
                                        setHTML(
                                            span,
                                            span.dataset.modOriginalContent,
                                        );
                                    }
                                    i++;
                                }
                ...
                if (shouldUpdate) {
                    ...
                    if (isWeekShown && cn("tijdlijn", 0)) {
                        const elements = cn(
                            "tijdlijn",
                            0,
                        ).parentElement.getElementsByTagName("sl-rooster-item");
                        if (
                            !n(elements[0]) &&
                            !n(
                                elements[
                            ...
                            let hour =
                                elements[
                                    ...
                                    .innerHTML;
                            hour = parseInt(
                                hour.substring(0, hour.length - 1),
                                10,
                            );
                            // Get top value of last hour
                            let lastHourTop =
                                elements[elements.length - 1].style.top;
                            lastHourTop = parseInt(
                                lastHourTop.substring(
                                    0,
                                    lastHourTop.length - 2,
                                ),
                                10,
                            );
                            // Get top value of time indicator
                            timeIndicatorTop = cn("tijdlijn", 0).style.top;
                            timeIndicatorTop = parseInt(
                                timeIndicatorTop.substring(
                                    0,
                                    timeIndicatorTop.length - 2,
                                ),
                                10,
                            );
                            ...
                            if (
                                !isNaN(hour) &&
                                !isNaN(lastHourTop) &&
                                !isNaN(timeIndicatorTop) &&
                                lastHourTop < timeIndicatorTop
                            ) {
                    ...
                    for (const element of parent.getElementsByTagName(
                        ...
                        if (
                            ...
                            let lessonTime = element
                                .getElementsByClassName("opacity-80")[0]
                                .parentElement.children[1].innerHTML.split(":");
                            lessonTime =
                                parseFloat(lessonTime[0]) +
                                parseFloat(lessonTime[1]) / 60;
                            let top;
                            if (isNaN(hour)) {
                    ...
                    if (isWeekShown) {
                        tn("sl-rooster-weken", 0).style.height =
                            Math.max(lastHour + 2, 5) * 84 - 54 + "px";
                        ...
                        for (const element of tn("sl-rooster-tijden", 0)
                            ...
                            if (n(span)) {
                                if (tn("sl-rooster-tijden", 0).children[1]) {
                                    setHTML(
                                        element,
                                        tn("sl-rooster-tijden", 0).children[1]
                                            .innerHTML,
                                    );
                                    span =
                                        element.getElementsByTagName("span")[0];
    ...
    function style() {
        ...
        if (get("bools").charAt(BOOL_INDEX.SCROLLBAR) == "0") {
            tn("head", 0).insertAdjacentHTML(
                "beforeend",
                '<style class="mod-style">html, body{scrollbar-width:none !important;}</style>',
            );
        }
        if (get("bools").charAt(BOOL_INDEX.TEXT_SELECTION) == "1") {
            tn("head", 0).insertAdjacentHTML(
                "beforeend",
                '<style class="mod-style">*{user-select:auto !important;}</style>',
            );
        }
        // General style
        tn("head", 0).insertAdjacentHTML(
            "beforeend",
            '<style class="mod-style">' +
                (get("bools").charAt(BOOL_INDEX.ROSTER_GRID) == "0"
                    ? "sl-rooster-week .uur{border-left:none !important;border-bottom:none !important;}"
                    : "") +
                "sl-rooster-dag.weekend,sl-rooster-dag.weekend .uur,sl-rooster-week .weekend,sl-rooster-week-header .weekend{background:transparent !important;background-color:transparent !important;}</style>",
        );
        tn("head", 0).insertAdjacentHTML(
            "beforeend",
            '<style class="mod-style">@media (min-width:767px){:root{--min-content-vh:calc(100vh - ' +
                (get("layout") == "4" ? "66px" : "74px") +
                ") !important;}}</style>",
        );
        // Font
        if (n(get("customfontname"))) {
            tn("head", 0).insertAdjacentHTML(
                "beforeend",
                '<style class="mod-style">@import url("' +
                    fontUrl +
                    '");*{font-family:"' +
                    get("fontname") +
                    '","Open Sans",sans-serif !important;' +
                    (get("fontname") == "Bebas Neue" ||
                    get("fontname") == "Oswald"
                        ? "letter-spacing:1px;"
                        : "") +
                    "}</style>",
            ...
            tn("head", 0).insertAdjacentHTML(
                ...
                '<style class="mod-style">@import url("' +
                    fontUrl +
                    '");@font-face{font-family:modCustomFont;src:url("' +
                    get("customfont") +
                    '");}*{font-family:modCustomFont,"Open Sans",sans-serif !important;}</style>',
            );
        }
        // Make sure everything is readable with background with 100% ui transparency
        if (
            get("layout") != 4 &&
            ((get("backgroundtype") == "image" && !n(get("background"))) ||
                get("backgroundtype") == "color" ||
                (get("backgroundtype") == "slideshow" &&
                    !n(get("background0"))) ||
                get("backgroundtype") == "live")
        ) {
            tn("head", 0).insertAdjacentHTML(
                "beforeend",
                '<style class="mod-style">hmy-switch-group:has(hmy-switch),sl-bericht-detail .header,sl-bericht-nieuw > .titel{border-radius:6px;padding:10px;background-color:var(--bg-neutral-none);}.content:has(sl-registraties){background:var(--mod-transparent);}sl-studiewijzer-week{border-bottom:2px solid var(--mod-transparent) !important;}sl-studiewijzer-dag{border-right:2px solid var(--mod-transparent) !important;}' +
                    (get("bools").charAt(BOOL_INDEX.ROSTER_GRID) == "1"
                        ? "sl-rooster-week .uur{border-left:2px solid var(--mod-transparent) !important;border-bottom:2px solid var(--mod-transparent) !important;}"
                        : "") +
                    ".container:has(sl-vakresultaten){padding-bottom:0 !important;}sl-vakresultaten{background-color:var(--bg-neutral-none);padding:20px !important; padding-bottom:40px !important;}hmy-geen-data > span{margin-top:20px;}hmy-geen-data{background:var(--mod-ui-transparent);padding:30px 60px;border-radius:24px;}</style>",
            );
        }
        ...
        if (get("layout") == 2 || get("layout") == 3 || get("layout") == 5) {
            ...
            tn("head", 0).insertAdjacentHTML(
                ...
                '<style class="mod-style">.menu-avatar .hamburger { flex-shrink: 1; min-width: 0; } @media (min-width:1280px){:root{--safe-area-inset-' +
                    ...
                    (get("layout") != 3 ? "left" : "right") +
                    ":0 !important;top: 0 !important;height:100% !important;overflow:hidden !important;border-bottom:0 !important;width:var(--safe-area-inset-" +
                    (get("layout") != 3 ? "left" : "right") +
                    ") !important;background:" +
                    get("primarycolor") +
                    " !important;color:" +
                    menuColor +
                    " !important;}sl-header > div:first-of-type{position:absolute !important;bottom:20px !important;left:12px !important;z-index:20 !important;width:calc(100% - 24px) !important;display:flex !important;justify-content:center !important;align-items:center !important;--bg-elevated-weakest:" +
                    highLightColor +
                    ";}}</style>",
            );
            tn("head", 0).insertAdjacentHTML(
                "beforeend",
                '<style class="mod-style">@media (max-width:1279px){#mod-logo-wrapper{width:100px;' +
                    (get("layout") != 3 ? "margin-left" : "left") +
                    ":calc((var(--safe-area-inset-" +
                    (get("layout") != 3 ? "left" : "right") +
                    ") - " +
                    (get("layout") != 3 ? "100px" : "115px") +
                    ") / 2);position:relative;}sl-tab-bar hmy-notification-counter{margin-top:-60px !important;margin-left:40px !important;}:root{--safe-area-inset-" +
                    (get("layout") != 3 ? "left" : "right") +
                    ": 100px !important;}#mod-background{width:calc(100% - var(--safe-area-inset-left) - var(--safe-area-inset-right) - 2 * " +
                    get("blur") +
                    " + 15px);}sl-tab-bar:first-of-type{position:fixed;top:0;" +
                    (get("layout") != 3 ? "left" : "right") +
                    ":0;border-top:none;width:" +
                    (get("layout") != 3
                    ...
                    highLightColor +
                    " !important;padding-top:0 !important;}sl-tab-bar:first-of-type sl-tab-item{--action-neutral-normal:" +
                    menuColor +
                    ";--action-primary-normal:" +
                    menuColor +
                    ";display:block !important;width:100%;position:relative !important;height:120px !important;}sl-header > div:first-of-type, .menu-avatar{position:fixed !important;bottom:20px !important;top:unset !important;" +
                    (get("layout") == 3 ? "right" : "left") +
                    ":12px !important;width:calc(var(--safe-area-inset-" +
                    (get("layout") != 3 ? "left" : "right") +
                    ") - 24px) !important;justify-content:center !important;display:flex !important;align-items:center !important;z-index:20 !important;--bg-elevated-weakest:" +
                    highLightColor +
                    ";}#mod-logo{--action-neutral-normal: " +
                    menuColor +
                    ";width:100%;height:60px;margin:20px 0;}}</style>",
            );
            tn("head", 0).insertAdjacentHTML(
                "beforeend",
                '<style class="mod-style">@media (max-width: 766px){sl-header:not(.with-back-button){border-bottom:none !important;--column-width: 100% !important;}sl-dagen-header{top:calc(' +
                    (get("layout") == 1 || get("layout") == 4
                        ? "64px"
                        : "0px") +
                    " + var(--safe-area-inset-top)) !important;border-bottom:none !important;}sl-berichten{background-color:var(--bg-neutral-none);}}</style>",
            );
            tn("head", 0).insertAdjacentHTML(
                "beforeend",
                '<style class="mod-style">.container{max-width:100%;}sl-header sl-tab-bar div.item span,sl-tab-bar sl-tab-item .item span{text-overflow:ellipsis;overflow:hidden;text-wrap:nowrap;margin-left:-5px;width:calc(100% + 10px);}sl-tab-bar sl-tab-item .item span{' +
                    (get("layout") == 3 ? "margin-left:5px;" : "") +
                    "width:calc(100% - 10px);text-align:center;}#mod-menu-resizer{width:12px;height:100%;top:0;" +
                    (get("layout") == 3 ? "left" : "right") +
                    ":-4px;position:absolute;cursor:ew-resize;}sl-tab-bar sl-tab-item .item{padding-bottom:0 !important;max-height:100%;}sl-rooster-dag.dag{width:calc(100vw - 170px) !important;}hmy-notification-counter span{margin:0 !important;}sl-header hmy-notification-counter{position:absolute;right:30px;top:20px;}sl-tab-bar:first-of-type sl-tab-item,sl-header sl-tab-item{position:relative !important;}.active-border-top,.active-border-bottom{top:0;height:100% !important;width:4px !important;position:absolute;" +
                    (get("layout") != 3 ? "right" : "left") +
                    ":0;}sl-sidebar{height:100% !important;}.active-border{display:none !important;}sl-rooster-week.week{width:calc(100% - 55px) !important;}sl-sidebar-page{padding-right:0 !important;}sl-header > div:first-of-type i{z-index:10000;--fg-on-primary-weak:" +
                    menuColor +
            ...
            tn("head", 0).insertAdjacentHTML(
                "beforeend",
                '<style class="mod-style">:root{--safe-area-inset-left:calc((100% - 1200px) / 2) !important;--safe-area-inset-right:calc((100% - 1200px) / 2) !important;}#mod-background{left:0;width:100%;}sl-home{position:relative;border:var(--thinnest-solid-neutral-normal);display:block;background:var(--bg-neutral-none) !important;' +
                    (get("ui") == 0
                        ? ""
                        : "backdrop-filter:blur(" + get("uiblur") + "px);") +
                    "}sl-rooster-week.week{width:calc(100% - 55px) !important;}</style>",
            );
        }
        ...
        if (get("bools").charAt(BOOL_INDEX.HIDE_MESSAGE_COUNT) == "1") {
            tn("head", 0).insertAdjacentHTML(
                "beforeend",
                '<style class="mod-style">hmy-notification-counter{display:none !important;}</style>',
            );
        }
        // Hide menu link text
        if (get("bools").charAt(BOOL_INDEX.MENU_PAGE_NAME) == "0") {
            tn("head", 0).insertAdjacentHTML(
                "beforeend",
                '<style class="mod-style">sl-header .item span{display:none !important;}@media (max-width:1279px){sl-tab-bar:first-of-type .item span{display:none !important;}}' +
                    (get("layout") == 2 ||
                    get("layout") == 3 ||
                    get("layout") == 5
                        ? "sl-tab-bar:first-of-type sl-tab-item i,sl-header .item i{padding-top:0!important;}"
                        : "sl-header .item i{padding-top:36px !important;}") +
                    "</style>",
            );
        }
        ...
        if (!n(get("customcss")) && get("customcss").trim() !== "") {
            ...
            const sanitizedCSS = get("customcss")
                .replace(/javascript:[^'"]+/gi, "") // Remove javascript: URLs
                .replace(/@import[^;\n]+/gi, ""); // Remove @import to prevent loading external CSS
            tn("head", 0).insertAdjacentHTML(
                "beforeend",
                '<style class="mod-style mod-custom-css">' +
                    sanitizedCSS +
                    "</style>",
            );
    ...
    function updateCheck() {
        ...
        if (!n(get("firstused"))) {
            ...
            if (n(get("version")) && !n(get("primarycolor"))) {
                modMessage(
                    "Somtoday update!",
                    "Somtoday heeft een grote update gekregen! Somtoday Mod is hier op voorbereid en heeft ook een update gekregen. De mod-instellingen zijn nu te vinden in een apart tabblad in de instellingen van Somtoday. Het is mogelijk dat je sommige instellingen opnieuw moet instellen.",
                    "Doorgaan",
                );
                id("mod-message-action1").addEventListener(
                    "click",
            ...
            if (savedVersion < 5.1) {
                ...
                if (n(get("nicknames"))) {
                    ...
                    const json = [];
                    const namearray = get("nicknames").split("|");
    ...
    function updateCssVariables() {
        ...
        if (get("ui") != 0 || get("backgroundtype") == "live") {
            tn("head", 0).insertAdjacentHTML(
                "beforeend",
                '<style id="mod-css-variables-2">sl-vakgemiddelden sl-dropdown,sl-cijfer-overzicht sl-dropdown{background:var(--bg-neutral-none);margin-top:-5px;margin-bottom:-5px;}' +
                    (get("uiblur") == 0
                        ? ""
                        : ".nieuw-bericht-form hmy-popup{top:70px !important;left:70px !important;}sl-plaatsingen,.nieuw-bericht-form,sl-header,sl-laatste-resultaat-item,sl-vakresultaat-item,.berichten-lijst,.vakken," +
                          (get("layout") == "4"
                              ? ""
                              : "sl-vakresultaten,hmy-geen-data,hmy-switch-group:has(hmy-switch),sl-bericht-detail .header,sl-bericht-nieuw > .titel,") +
                          ".headers-container,.tabs,sl-studiewijzer-week:has(.datum.vandaag),#mod-top-menu,sl-home > * > sl-tab-bar.show,sl-dagen-header,sl-scrollable-title,sl-studiewijzer-weken-header,sl-cijfer-overzicht-voortgang>div,sl-rooster-tijden{backdrop-filter:blur(" +
                          get("uiblur") +
                          "px);}") +
                    "@media(max-width:767px){sl-laatste-resultaat-item{backdrop-filter:none;}sl-laatsteresultaten{backdrop-filter:blur(" +
                    get("uiblur") +
                    "px);}}:root, :root.dark.dark {--thinnest-solid-neutral-strong:1px solid transparent !important;--text-weakest:var(--text-weak);--border-neutral-normal:transparent;" +
                    (darkmode && get("ui") > 0.9 ? "--text-weak:#fff;" : "") +
                    "--bg-neutral-none:" +
                    (darkmode
                        ? "rgba(0,0,0," + (1 - get("ui") / 100) + ")"
                        : "rgba(255,255,255," + (1 - get("ui") / 100) + ")") +
                    ";--bg-neutral-weakest:" +
                    (darkmode
                        ? "rgba(0, 0, 0, " + (1 - get("ui") / 100) + ")"
                        : "rgba(255, 255, 255, " +
                          (1 - get("ui") / 100) +
                          ")") +
                    ";}.mod-multi-choice,input:not(:hover):not(:focus):not(.mod-color-textinput):not(.ng-pristine):not(.ng-dirty),textarea:not(:hover):not(:focus):not(.ng-pristine):not(.ng-dirty),.select-selected{border:1px solid rgba(0,0,0,0.1) !important;}hmy-toggle .toggle:not(:has(input:checked)) .slider{border:2px solid rgba(0,0,0,0.1) !important;}sl-dag-header-tab,.periode-icon{background:none !important;}@media (max-width:767px){sl-vakgemiddelden sl-dropdown,sl-cijfer-overzicht sl-dropdown{margin-top:10px;}}</style>",
            );
        }
        // If at least one of the colors is not set to the default value, modify Somtoday color variables
        let css =
            (darkmode
                ? "#mod-setting-panel ::placeholder{ color: var(--action-neutral-normal) !important; }"
                : "") +
            " :root, :root.dark.dark { --mod-transparent: rgba(" +
            (darkmode ? "0,0,0,0.3" : "255,255,255,0.3") +
            "); --mod-ui-transparent: rgba(" +
            (darkmode ? "0,0,0" : "255,255,255") +
            "," +
            (1 - get("ui") / 100) +
            "); --mod-semi-transparant: " +
            (tn("html", 0).classList.contains("night")
                ? "#000"
                : darkmode
                  ? "rgba(0,0,0,0.35)"
                  : "rgba(255,255,255,0.65)") +
            "; --mod-border-neutral-normal: rgb(" +
            (darkmode ? "55,64,72" : "208,214,220") +
            ");";
        if (
            get("primarycolor") != "#0067c2" ||
            get("secondarycolor") != "#e69b22"
        ) {
            const rgbcolor = hexToRgb(get("primarycolor"));
            let cssVariables = "";
            const blueBrightness = [
                "241",
                "220",
                "198",
                "169",
        ...
        tn("head", 0).insertAdjacentHTML(
            "beforeend",
            `<style id="mod-css-variables">${css}</style>`,
        );
    }
    ...
    function modMessage(
        ...
        const element = n(id("somtoday-mod"))
            ? tn("body", 0)
            : id("somtoday-mod");
        element.insertAdjacentHTML(
            "afterbegin",
            '<div id="mod-message" class="mod-animation-playing"><center><div onclick="event.stopPropagation();"><h2>' +
                title +
                "</h2><p>" +
                description +
                "</p>" +
                (n(link1)
                    ? ""
                    : '<a id="mod-message-action1" class="mod-message-button' +
                      (red1 ? " mod-button-discouraged" : "") +
                      '" tabindex="0">' +
                      link1 +
                      "</a>") +
                (n(link2)
                    ? ""
                    : '<a id="mod-message-action2" class="mod-message-button' +
                      (red2 ? " mod-button-discouraged" : "") +
                      '" tabindex="0">' +
                      link2 +
                      "</a>") +
                "</div></center></div>",
        ...
        if (!n(link1)) {
            ...
            id("mod-message-action1").addEventListener(
                ...
                () => {
                    ...
                    if (tn("sl-modal", 0)) {
                        tn("sl-modal", 0).removeAttribute("inert");
                    }
                },
                { once: true },
            );
        ...
        if (!n(link2)) {
            ...
            if (noBackgroundClick == null) {
                id("mod-message").addEventListener(
                    "click",
                    () => {
                        if (!n("mod-message-action2")) {
                            id("mod-message-action2").click();
                        }
                    },
                    { once: true },
                );
            ...
            id("mod-message").addEventListener(
                "click",
                () => {
                    if (!n("mod-message-action1")) {
                        id("mod-message-action1").click();
                    }
                },
                { once: true },
            );
    ...
    function profilePicture() {
        ...
        if (!n(get("profilepic"))) {
            ...
            tn("head", 0).insertAdjacentHTML(
                "beforeend",
                '<style id="mod-profile-picture">hmy-avatar>.container{border-radius:var(--border-radius);background:url(\'' +
                    get("profilepic") +
                    "') center / cover !important;}.foto, .initials{opacity:0 !important;}</style>",
            );
        }
        ...
        if (!n(get("nicknames"))) {
            ...
            for (const nickname of namearray) {
                ...
                if (real != "") {
                    ...
                    const regexWhichPreservesIcons = new RegExp(
                        "(.*?)" +
                            real.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") +
                            "(.*?<hmy-?.*)",
                        "g",
                    ...
                    for (const element of cn("ontvangers")) {
                        if (
                            !element.classList.contains("mod-nickname") &&
                            !element.classList.contains("ellipsis") &&
                            !element.classList.contains("input-veld") &&
                            (n(element.children[0]) ||
                                !element.children[0].classList.contains(
                                    "zoekveld",
                                ))
                        ) {
                            setHTML(
                            ...
                            const text = element.innerHTML.replace(
                                element.innerHTML.indexOf("<hmy-") != -1
                                    ? regexWhichPreservesIcons
                                    : regex,
                                "$1" + nick + "$2",
                            );
                            if (text != element.innerHTML) {
                    ...
                    // Sender field in opened message (can be a teacher name or student name)
                    for (const element of cn(
                        "text text-content-smallest-semi",
                    )) {
                        if (!element.classList.contains("mod-nickname")) {
                            const text = element.innerHTML.replace(
                                regex,
                    ...
                    for (const element of cn("docent")) {
                        if (
                            ...
                            const text = element
                                ...
                                .innerHTML.replace(regex, "$1" + nick + "$2");
                            if (
                                text !=
                                element.getElementsByTagName("span")[0]
                                    .innerHTML
                    ...
                    for (const element of tn("hmy-internal-tag")) {
                        if (
                            ...
                            !element
                                .getElementsByTagName("span")[0]
                                .classList.contains("mod-nickname") &&
                            element.getElementsByTagName("span")[0]
                        ) {
                            if (
                                nickname[2] &&
                                element.innerText == nickname[2]
                            ) {
                                element
                                    .getElementsByTagName("span")[0]
                                    .classList.add("mod-nickname");
                                setHTML(
                                    element.getElementsByTagName("span")[0],
                                    "",
                                );
    ...
    function userName() {
        if (
            ...
            for (const element of cn("van")) {
                if (element.innerHTML.indexOf(get("realname")) != -1) {
                    ...
                    element.append(
                        document
                            .createRange()
                            .createContextualFragment(get("username")),
                    );
                }
            }
            // Receiver field in opened message (can be a teacher name, student name, group name or "Somtoday (automatisch)")
            ...
            // Everything is stored as "<span>FirstName&nbsp;</span> <span>Lastname</span>" and this needs to be parsed
            for (const element of cn("ontvangers")) {
                const search = get("realname").replaceAll(
                    " ",
                    "&nbsp;</span> <span>",
                );
                if (element.innerHTML.indexOf(search) != -1) {
                    element.innerHTML = element.innerHTML.replaceAll(
                        search,
                        get("username").replaceAll(" ", "&nbsp</span> <span>"),
                    );
                }
            }
            for (const element of tn("hmy-internal-tag")) {
                if (
                    !n(element.getElementsByTagName("span")[0]) &&
                    element
                        .getElementsByTagName("span")[0]
                        .innerHTML.indexOf(get("realname")) != -1
                ) {
                    element.getElementsByTagName("span")[0].innerHTML = element
                        .getElementsByTagName("span")[0]
                        .innerHTML.replaceAll(get("realname"), get("username"));
                }
            }
            ...
            for (const element of tn("sl-html-content")) {
                if (
                    !n(element.children[0]) &&
                    element.children[0].innerHTML.indexOf(get("realname")) != -1
                ) {
                    element.children[0].innerHTML =
                        element.children[0].innerHTML.replaceAll(
                            get("realname"),
                            get("username"),
                        );
                }
    ...
    function modLogo() {
        if (get("layout") == 2 || get("layout") == 3 || get("layout") == 5) {
            ...
            tabBars.forEach((tabBar) => {
                if (!tabBar.dataset.modWheelAttached) {
                    tabBar.dataset.modWheelAttached = "true";
            ...
            const avatar = cn("menu-avatar", 0);
            if (avatar && !avatar.dataset.modWheelAttached) {
                avatar.dataset.modWheelAttached = "true";
                avatar.addEventListener(
                    "wheel",
                    (e) => {
                        const tb =
                            (tn("sl-header", 0) &&
                                tn("sl-header", 0).getElementsByTagName(
                                    "sl-tab-bar",
                                )[0]) ||
                            tn("sl-tab-bar", 0);
            ...
            if (n(id("mod-menu-resizer")) && tn("sl-tab-bar", 0)) {
                tn("sl-tab-bar", 0).insertAdjacentHTML(
                    "beforeend",
                    '<div id="mod-menu-resizer"></div>',
                );
                let moving = false;
                if (get("layout") == 3) {
                ...
                id("mod-menu-resizer").addEventListener("mousedown", (e) => {
                    moving = true;
                });
                id("mod-menu-resizer").addEventListener("touchstart", (e) => {
                    moving = true;
                });
        ...
        if (
            cn("mod-logo-hat-clicked", 0) ||
            cn("mod-logo-decoration-clicked", 0)
        ) {
            return;
        }
        ...
        if (get("layout") == 2 || get("layout") == 3 || get("layout") == 5) {
            const logoHTML =
                '<div id="mod-logo-wrapper">' +
                (get("bools").charAt(BOOL_INDEX.MOD_LOGO) == "0"
                    ? window.getIcon("logo", null, menuColor, ' id="mod-logo"')
                    : window.logo(
                          "mod-logo",
                          '" data-clicks="' +
                              (n(id("mod-logo"))
                                  ? "0"
                                  : id("mod-logo").dataset.clicks),
                          "var(--action-neutral-normal)",
                      )) +
                "</div>";
            if (
                n(id("mod-logo")) &&
                tn("sl-header", 0) &&
                tn("sl-header", 0).getElementsByTagName("sl-tab-bar")[0]
            ) {
                tn("sl-header", 0)
                    .getElementsByTagName("sl-tab-bar")[0]
                    .insertAdjacentHTML("afterbegin", logoHTML);
            } else if (n(id("mod-logo")) && tn("sl-tab-bar", 0)) {
                tn("sl-tab-bar", 0).insertAdjacentHTML("afterbegin", logoHTML);
            } else {
                return;
            }
            id("mod-logo").addEventListener("click", () => {
                logoClicks += 1;
                if (logoClicks == 10) {
                    logoClicks = 0;
                    ...
                    setTimeout(() => {
                        id("blue-screen-of-death").addEventListener(
                            "click",
                            function () {
                                tn("body", 0).style.setProperty(
                                    "overflow-y",
        ...
        if (
            get("bools").charAt(BOOL_INDEX.EVENTS) == "1" &&
            n(id("mod-logo-hat")) &&
            !tn("body", 0).classList.contains("mod-logo-hat-hidden")
        ) {
            ...
            const birthday =
                parseInt(
                    get("birthday").charAt(0) + get("birthday").charAt(1),
                    10,
                ) == dayInt &&
                parseInt(
                    get("birthday").charAt(3) + get("birthday").charAt(4),
                    10,
                ) == monthInt;
            ...
            if (
                (get("layout") == 1 || get("layout") == 4) &&
                tn("sl-header", 0) &&
                (birthday || christmas || sinterklaas)
            ) {
                tn("sl-header", 0).style.overflow = "hidden";
                tn("sl-header", 0).insertAdjacentHTML(
                    "beforeend",
                    window.logo(
                        "mod-logo-inserted",
                        null,
                        "var(--action-neutral-normal)",
                        "position:absolute;width:50px;height:54px;right:25px;bottom:-15px;z-index:-1;transition:bottom 0.4s ease 0.3s;",
                    ),
                );
        ...
        if (
            get("bools").charAt(BOOL_INDEX.EVENTS) == "1" &&
            n(id("mod-logo-decoration")) &&
            !tn("body", 0).classList.contains("mod-logo-decoration-hidden")
        ) {
            let insertElement = id("mod-logo");
    ...
    function startLiveWallpaper(preview = false, col1, col2, col3) {
        ...
        if (!preview) {
            ...
            tn("body", 0).insertAdjacentHTML(
                "beforeend",
                '<canvas id="mod-background-live"></canvas>',
            );
        }
    ...
    function setBackground() {
        ...
        stopLiveWallpaper();
        if (
            (n(get("backgroundtype")) || get("backgroundtype") == "image") &&
            !n(get("background"))
        ) {
            // Unfortunately, localStorage can only save strings, so check for 'false' too
            if (
                get("isbackgroundvideo") &&
                get("isbackgroundvideo") != "false"
            ) {
                id("somtoday-mod", 0).insertAdjacentHTML(
                    "beforeend",
                    '<video id="mod-background" src="' +
                        get("background") +
                        '" style="' +
            ...
            tn("head", 0).insertAdjacentHTML(
                "beforeend",
                '<style id="mod-background-style">#mod-background{' +
                    (get("layout") == 5
                        ? "--safe-area-inset-top: 0px !important;"
                        : "") +
                    "pointer-events:none;user-select:none;position:fixed;left:calc(var(--safe-area-inset-left) - " +
                    get("blur") +
                    ");top:calc(var(--safe-area-inset-top) - " +
                    get("blur") +
                    ");width:calc(100% - var(--safe-area-inset-left) - var(--safe-area-inset-right) - 2 * " +
                    get("blur") +
                    ");height:calc(100% - var(--safe-area-inset-top) - var(--safe-area-inset-bottom) - 2 * " +
                    get("blur") +
                    ");object-fit:cover;z-index:-1;}</style>",
            );
        } else if (
            get("backgroundtype") == "color" &&
            ...
            id("somtoday-mod", 0).insertAdjacentHTML(
                "beforeend",
                '<div id="mod-backgroundcolor"></div>',
            );
            tn("head", 0).insertAdjacentHTML(
                "beforeend",
                '<style id="mod-background-style">#mod-backgroundcolor{pointer-events:none;user-select:none;position:fixed;left:0;width:100%;top:0;height:100%;object-fit:cover;z-index:-1;background:' +
                    get("backgroundcolor") +
                    ";}</style>",
            );
        } else if (
            ...
            id("somtoday-mod", 0).insertAdjacentHTML(
                "beforeend",
                '<img id="mod-backgroundslide" src="' +
                    get("background" + randomChoice) +
                    '">',
            );
            tn("head", 0).insertAdjacentHTML(
                "beforeend",
                '<style id="mod-background-style">#mod-backgroundslide{pointer-events:none;user-select:none;position:fixed;left:0;width:100%;top:0;height:100%;object-fit:cover;z-index:-1;}</style>',
            );
    ...
    function getBackgroundFilters(fromSavedSettings = true) {
        ...
        for (const filterProperty of properties) {
            const value = fromSavedSettings
                ? get(filterProperty)
                : id(filterProperty).value + id(filterProperty).dataset.unit;
            if (value != null) {
                // Add to filter
                // Extensions can't save data with "hue-rotate" as key because it contains a hyphen
                filter +=
                    " " +
                    (filterProperty == "huerotate"
                        ? "hue-rotate"
                        : filterProperty) +
                    "(" +
                    value +
    ...
    function browserSettings() {
        if (!n(get("title"))) {
            if (n(tn("title", 0).dataset.modOriginalTitle)) {
                tn("title", 0).dataset.modOriginalTitle = tn(
                    "title",
                    0,
                ).innerHTML;
            }
    ...
    (() => {
        ...
        function startConfettiInner() {
            ...
            window.requestAnimFrame = (() =>
                ...
                ((callback) => window.setTimeout(callback, 16.6666667)))();
            ...
            if (canvas === null) {
                ...
                canvas.setAttribute(
                    "style",
                    "display:block;z-index:999999;pointer-events:none;position:fixed;top:0;left:0;transition:0.3s opacity ease;",
                );
    ...
    function congratulations() {
        ...
        if (
            get("bools").charAt(BOOL_INDEX.CONGRATULATIONS) == "1" &&
            year + "-" + (month + 1) + "-" + dayInt != get("lastused")
        ) {
            ...
            const yeardifference = parseInt(
                (lastused - firstused) / 1000 / 60 / 60 / 24 / 365,
                10,
            );
            // Get birthday date
            const date = new Date();
                ...
                congratstext = [
                    "Bedankt" +
                        (n(get("username"))
                            ? n(get("realname"))
                                ? ""
                                : ", " + get("realname").replace(/ .*/, "")
                            : ", " + get("username").replace(/ .*/, "")) +
                        "!",
                    "Je gebruikt Somtoday Mod nu al " +
                        yeardifference +
                        " jaar. Bedankt!",
                ];
            ...
            if (birthdayday == dayInt && birthdaymonth == month + 1) {
                const randomDescriptionArray = [
                    "Maak er maar een mooie dag van!",
                    "Gefeliciteerd! Je bent weer een jaartje dichterbij je afstuderen!",
                    "Geniet ervan! Mogen al je dromen uitkomen!",
                    "Jaja, je bent weer jarig! Van harte gefeliciteerd!",
                    '"School is raar, hier is een schaar" ~ iemand die ooit jarig was',
                ];
                congratstext = [
                    "Fijne verjaardag!",
                    randomDescriptionArray[
                        Math.floor(
                            Math.random() * randomDescriptionArray.length,
                        )
                    ],
                ];
            }
    ...
    function downloadGrades() {
        ...
        if (n(tn("sl-resultaat-item", 0)) || cijferoverzicht) {
            ...
            modMessage(
                "Hoeveel cijfers wil je downloaden?",
                'Kies het aantal cijfers dat je wil downloaden (1-25).<div class="br"></div><input id="mod-grades-amount" type="number" min="1" max="25" step="1" onkeyup="if (this.value != \'\') { this.value = Math.floor(this.value); } if (this.value < 1 && this.value != \'\') { this.value = 1; } else if (this.value > 25) { this.value = 25; }"/>',
                "Doorgaan",
                "Annuleren",
            );
        ...
        async function download() {
            ...
            if (cijferoverzicht) {
                ...
                for (const section of cn("periode-header")) {
                    ...
                    const count = parseInt(
                        section.style.getPropertyValue("--aantal-items"),
                        10,
                    );
                ...
                const columns = cn(
                    "vak-gemiddelde-header",
                )[0].getElementsByTagName("th");
                for (i = 1; i < columns.length; i++) {
                    items[i - 1].column = columns[i].innerHTML.trim();
                    items[i - 1].gemiddelde =
                        columns[i].classList.contains("gemiddelde");
                ...
                totalWidth = 1000;
                number = tn("sl-resultaat-item", 0)
                    ? Math.min(
                          Math.round(parseFloat(id("mod-grades-amount").value)),
                          tn("sl-resultaat-item").length,
                      )
                    : tn("sl-vakgemiddelde-item").length;
                if (isNaN(number)) {
                    number = 1;
                }
            }

            const cijferOverzichtPeriod =
                tn("sl-dropdown", 0) && tn("sl-dropdown", 0).ariaLabel
            ...
            modMessage(
                "Downloaden...",
                "Somtoday Mod is bezig met het genereren van je afbeelding. Dit kan even duren...",
            );
            // Construct HTML
            const font = "Kanit, Arial, sans-serif, Tahoma";
            const headerBackground = darkmode ? "#2b3c63" : "#0099ff";
            const headerText = cijferoverzicht
                ? cijferOverzichtPeriod
                : n(tn("sl-vakgemiddelde-item", 0))
                  ? "Mijn cijfers"
                  : "Laatste rapportcijfers";
            let html = `
                <div style="
                    width: ${totalWidth}px;
                    height: 218px;
            ...
            if (cijferoverzicht) {
                ...
                for (const item of items) {
                    ...
                    if (
                        item.column &&
                        item.column.replace(/\s/g, "") != "" &&
                        item.column.length <= 3
                    ) {
                        content = item.column;
                    }
                ...
                for (const vak of vakken) {
                    ...
                    for (const item of items) {
                        ...
                        const el =
                            vak.getElementsByTagName("td")[item.index + 1];
                ...
                for (let i = 0; i < number; i++) {
                    ...
                    if (tn("sl-vakgemiddelde-item", i)) {
                        averagePageGradeIndex =
                            tn(
                                "sl-vakgemiddelde-item",
                                i,
                            ).getElementsByClassName("cijfer").length - 1;
                    }
                    const subjectIcon = n(tn("sl-resultaat-item", 0))
                        ? tn("sl-vakgemiddelde-item", i).getElementsByTagName(
                              "svg",
                          )[0].innerHTML
                        : tn("sl-resultaat-item", i).getElementsByTagName(
                              "svg",
                          )[0].innerHTML;
                    const subjectName = n(tn("sl-resultaat-item", 0))
                        ...
                        : tn("sl-resultaat-item", i).getElementsByClassName(
                              "titel",
                          )[0].innerHTML;
                    ...
                    const red =
                        (tn("sl-resultaat-item", i) &&
                            ...
                            tn("sl-resultaat-item", i)
                                .getElementsByClassName("cijfer")[0]
                                .classList.contains("onvoldoende")) ||
                        (tn("sl-vakgemiddelde-item", i) &&
                            ...
                            tn("sl-vakgemiddelde-item", i)
                                .getElementsByClassName("cijfer")
                                [averagePageGradeIndex].classList.contains(
                                    "onvoldoende",
                                ));
                    const green =
                        (tn("sl-resultaat-item", i) &&
                            ...
                            tn("sl-resultaat-item", i)
                                .getElementsByClassName("cijfer")[0]
                                .classList.contains("ruimvoldoende")) ||
                        (tn("sl-vakgemiddelde-item", i) &&
                            tn(
                                "sl-vakgemiddelde-item",
                                i,
            ...
            tn("body", 0).insertAdjacentHTML(
                "beforeend",
                `<canvas id="mod-grade-canvas" width="${totalWidth}" height="${totalHeight}" style="display:none;"></canvas>`,
            );
            const canvas = id("mod-grade-canvas");
            const ctx = canvas.getContext("2d");
            // Use data urls for font in SVG
            const kanit = await window.getResourceAsBase64(
                "fonts/Kanit-ExtraLight.woff2",
            );
            ...
            const svg =
                ...
                '" height="' +
                canvas.height +
                '"><defs><style type="text/css">@font-face{font-family:Kanit;src:url(\'' +
                kanit +
                '\')}</style></defs><foreignObject width="100%" height="100%"><div xmlns="http://www.w3.org/1999/xhtml">' +
                html +
    ...
    function gradeGraphs(recapData) {
        if (!n(recapData)) {
            ...
            const recapCanvas = document.getElementById("recap-chart");
            const recapCtx = recapCanvas.getContext("2d");
            const recapGradient = recapCtx.createLinearGradient(0, 0, 0, 300);
        ...
        if (
            n(document.getElementById("mod-chart-1")) ||
            n(document.getElementById("mod-chart-2"))
        ) {
            return;
        }
        ...
        for (const element of tn("sl-resultaat-item")) {
            if (
                !n(element.getElementsByClassName("cijfer")[0]) &&
                !n(element.getElementsByClassName("cijfer")[0].children[0])
            ) {
                if (
                    element
                        .getElementsByClassName("weging")[0]
                        .innerHTML.replace(",", ".")
                        .match(/[0-9]+(\.[0-9]+)?/) == null
                ) {
                    continue;
                }
                const weging = parseFloat(
                    element
                        .getElementsByClassName("weging")[0]
                        .innerHTML.replace(",", ".")
                        .match(/[0-9]+(\.[0-9]+)?/)[0],
                ...
                let dateObject;
                const dateString = n(
                    element.getElementsByClassName("subtitel")[0],
                )
                    ? ""
                    : element.getElementsByClassName("subtitel")[0].innerHTML;
                if (dateString.indexOf("Vandaag") != -1) {
                    ...
                    let englishDateString = dateString
                        .replace("mrt", "mar")
                        .replace("mei", "may")
                        .replace("mei", "may")
                        .replace("okt", "oct");
                ...
                if (isNaN(grade) && weging != 0) {
                    const letterbeoordelingen = parseJSON(
                        get("letterbeoordelingen"),
                    );
                    const letter = element
                        .getElementsByClassName("cijfer")[0]
                        .children[0].innerHTML.replace(",", ".");
                    if (
                        letterbeoordelingen == null ||
                        letterbeoordelingen[letter] == null
                    ) {
        ...
        let suggestionText =
            "Er zijn nog niet genoeg cijfers om een analyse te maken.";
        if (totalWeight > 0) {
            ...
            let trendText = "";
            if (trend == "stijgend")
                trendText = " <br>🚀 Je laatste cijfers zitten in de lift!";
            else if (trend == "dalend")
                trendText =
                    " <br>📉 Pas op, je laatste cijfers zijn wat lager dan gemiddeld.";
            ...
            datasets: [
                {
                    label:
                        n(cn("vaknaam", 0)) ||
                        n(cn("vaknaam", 0).getElementsByTagName("span")[0])
                            ? ""
                            : cn("vaknaam", 0).getElementsByTagName("span")[0]
                                  .innerHTML,
                    lineTension: 0,
        ...
        for (let i = 0; i < points.length; i++) {
            ...
            values.push(
                Math.floor((totalGrades / rollingTotalWeight) * 100) / 100,
            );
        }
        chartdata = {
            labels: dates,
    ...
    function showLetterbeoordelingenMessage(letter) {
        ...
        if (
            letter != null &&
            !n(letterbeoordelingen) &&
            letterbeoordelingen[letter] == "-"
        ) {
            return;
        }
        ...
        for (const [key, value] of Object.entries(list)) {
            listHTML +=
                ...
                key +
                (value[0] ? " (" + value[0] + ")" : "") +
                '</label><input class="mod-letterbeoordelingen-letter" value="' +
                value[1] +
                '" data-mod-letter="' +
                key +
        ...
        modMessage(
            "Letterbeoordelingen",
            'Bij sommige scholen tellen letterbeoordelingen (O, V, G, etc) mee voor je gemiddelde. Helaas verschilt het per school hoeveel een letter als cijfer waard is. Vul hieronder in hoeveel elke letter waard is.</p><div id="mod-letterbeoordelingen">' +
                listHTML +
                "</div><p>",
            "Opslaan",
            "Sluiten",
    ...
    function calculateAverage() {
        ...
        for (const element of document.getElementsByTagName(
            ...
            if (
                !n(element.getElementsByClassName("cijfer")[0]) &&
                !n(element.getElementsByClassName("cijfer")[0].children[0]) &&
                !n(element.getElementsByClassName("weging")[0])
            ) {
                ...
                const tempWeight = parseFloat(
                    element
                        .getElementsByClassName("weging")[0]
                        .innerHTML.replace(",", ".")
                        .match(/[0-9]+(\.[0-9]+)?/)[0],
                );
                const tempGrade = parseFloat(
                    element
                        .getElementsByClassName("cijfer")[0]
                        .children[0].innerHTML.replace(",", "."),
                );
    ...
    function insertCalculationTool() {
        if (
            n(id("mod-grade-calculate")) &&
            tn("sl-resultaat-item", 0) &&
            get("bools").charAt(BOOL_INDEX.CALCULATION_TOOL) == "1"
        ) {
            ...
            id("mod-grade-one-three").addEventListener("click", () => {
                const averageData = calculateAverage();
                // Calculate grade needed for chosen average
                const chosenAverage = parseFloat(id("mod-grade-one-one").value);
                const chosenWeight = parseFloat(id("mod-grade-one-two").value);
                if (isNaN(chosenAverage) || isNaN(chosenWeight)) {
                    id("mod-grade-one-three").value = "Berekenen";
                } else {
                    id("mod-grade-one-three").value = (
                        Math.ceil(
                            ((chosenAverage *
                                (chosenWeight + averageData.weight) -
                                averageData.total) /
                                chosenWeight) *
                                100,
                        ) / 100
            ...
            id("mod-grade-two-three").addEventListener("click", () => {
                const averageData = calculateAverage();
                // Calculate average with chosen grade added
                const chosenGrade = parseFloat(id("mod-grade-two-one").value);
                const chosenWeight = parseFloat(id("mod-grade-two-two").value);
                if (isNaN(chosenGrade) || isNaN(chosenWeight)) {
                    ...
                    id("mod-grade-two-three").value = (
                        Math.floor(
                            (averageData.total / averageData.weight) * 100,
                        ) / 100
                    )
                        .toFixed(2)
                        .toString()
    ...
    function insertGradeDownloadButton() {
        if (
            (tn("sl-resultaat-item", 0) ||
                tn("sl-vakgemiddelde-item", 0) ||
                tn("sl-cijfer-overzicht", 0)) &&
            n(tn("sl-vakresultaten", 0)) &&
            get("bools").charAt(BOOL_INDEX.GRADE_DOWNLOAD_BTN) == "1"
        ) {
            if (
                n(id("mod-grades-download-computer")) &&
                tn("hmy-switch-group", 0)
            ) {
                tn("hmy-switch-group", 0).insertAdjacentHTML(
                    "beforeend",
                    '<a id="mod-grades-download-computer" class="mod-grades-download">' +
                        window.getIcon(
                            "download",
                            null,
                            "var(--fg-primary-normal)",
                        ) +
                        "</a>",
                );
                id("mod-grades-download-computer").addEventListener(
                    "click",
                    downloadGrades,
                );
            }
            if (
                n(id("mod-grades-download-mobile")) &&
                cn("tabs ng-star-inserted", 0)
            ) {
                if (document.documentElement.clientWidth > 767) {
                    cn("tabs ng-star-inserted", 0)
                        .getElementsByClassName("filler")[0]
                        .insertAdjacentHTML(
                            "beforeend",
                            '<a id="mod-grades-download-mobile" class="mod-grades-download">' +
                                window.getIcon(
                                    "download",
                                    null,
                                    "var(--fg-primary-normal)",
                    ...
                    id("mod-grades-download-mobile").addEventListener(
                        "click",
                        downloadGrades,
                    );
                } else {
                    tn("sl-scrollable-title", 0).insertAdjacentHTML(
                        "beforeend",
                        '<a id="mod-grades-download-mobile" class="mod-grades-download">' +
                            window.getIcon(
                                "download",
    ...
    function subjectGradesPage() {
        if (tn("sl-vakresultaten", 0)) {
            ...
            const firstCondition =
                n(id("mod-grades-graphs")) &&
                get("bools").charAt(BOOL_INDEX.SUBJECT_GRAPHS) == "1";
            const secondCondition =
                id("mod-grades-graphs") &&
                ((examPage &&
                    id("mod-grades-graphs").dataset.exams == "false") ||
                    (!examPage &&
                        id("mod-grades-graphs").dataset.exams == "true"));
            if (firstCondition || secondCondition) {
                ...
                tn("sl-vakresultaten", 0).insertAdjacentHTML(
                    ...
                    '<div id="mod-cijferanalyse">' +
                        (get("bools").charAt(BOOL_INDEX.GRADE_ANALYSIS) == "1"
                            ? '<h3 style="margin-top: 40px;">Cijferanalyse</h3>' +
                            ...
                            : "") +
                        "</div>" +
                        '<div id="mod-grades-graphs" data-exams="' +
                        (examPage ? "true" : "false") +
                        '">' +
                        "<h3>Mijn " +
                        (examPage ? "examen" : "") +
                        'cijfers</h3><div><canvas id="mod-chart-1"></canvas></div>' +
                        "<h3>Mijn " +
                        (examPage ? "examen" : "") +
                        'gemiddelde</h3><div><canvas id="mod-chart-2"></canvas></div>' +
                        "</div>",
                );
                setTimeout(() => {
                    execute([gradeGraphs]);
    ...
    function somtodayRecap() {
        ...
        if (
            id("somtoday-recap") &&
            n(id("somtoday-recap-wrapper")) &&
            (tn("sl-vakresultaten", 0) ||
                id("somtoday-recap").nextElementSibling.tagName ==
                    "HMY-SWITCH-GROUP")
        ) {
            tryRemove(id("somtoday-recap"));
        }
        if (id("mod-recap-year")) {
            id("mod-recap-year").innerText =
                tn("sl-dropdown", 0) && tn("sl-dropdown", 0).ariaLabel
                    ? tn("sl-dropdown", 0).ariaLabel.replace(
                          /^[^/]+\/(\d+)/,
                          "$1",
                      )
                    : year;
        ...
        if (
            ...
            tn("hmy-switch-group", 0).insertAdjacentHTML(
                "afterend",
                '<div id="somtoday-recap"><h3>Somtoday Recap' +
                    window.logo(
                        null,
                        null,
                        "#fff",
                        "height:1em;width:fit-content;margin-left:10px;transform:translateY(2px);",
                    ) +
                    '</h3><p>Bekijk hier jouw jaaroverzicht van <span id="mod-recap-year">' +
                    recapYear +
                    '</span>.</p><div id="somtoday-recap-arrows">' +
                    window.getIcon(
                        "chevron-right",
                        null,
                        "#fff",
            ...
            // Open recap on click
            id("somtoday-recap").addEventListener("click", async () => {
                music.currentTime = 0;
                music.play();
                music.volume = 1;
                isRecapping = true;
                    ...
                    recapClose.addEventListener("click", () => {
                        if (document.documentElement.clientWidth < 1280) {
                            window.location.href =
                                "https://leerling.somtoday.nl/cijfers";
                            return;
                        }
                        closing = true;
                ...
                for (const element of cn("vak-row")) {
                    ...
                    if (element.getElementsByClassName("naam")[0]) {
                        subject.name =
                            element.getElementsByClassName("naam")[0].innerText;
                    }
                    if (
                        element.getElementsByTagName("hmy-vak-icon")[0] &&
                        element
                            .getElementsByTagName("hmy-vak-icon")[0]
                            .getElementsByTagName("svg")[0]
                    ) {
                        subject.icon = element
                            .getElementsByTagName("hmy-vak-icon")[0]
                            .getElementsByTagName("svg")[0].outerHTML;
                    }
                    const averageElements =
                        element.getElementsByClassName("cijfer gemiddelde");
                    if (averageElements.length >= 1) {
                        ...
                        for (const gemiddelde of averageElements) {
                            if (
                                gemiddelde.innerText.indexOf(",") != -1 ||
                                gemiddelde.innerText.indexOf(".") != -1
                            ) {
                        ...
                        if (!average) {
                            average =
                                averageElements[averageElements.length - 1]
                                    .innerText;
                        }

                        subject.average = isNaN(parseFloat(average))
                            ? average
                            : parseFloat(average);
                        if (!isNaN(parseFloat(subject.average))) {
                            totalAverageGrades += Math.round(
                    ...
                    for (const cijfer of element.getElementsByClassName(
                        "cijfer",
                    )) {
                        if (cijfer.innerText == "") {
                            continue;
                        ...
                        const grade = parseFloat(
                            cijfer.innerText.replace(",", "."),
                        );
                        const weging = cijfer.ariaLabel
                            ? parseFloat(
                                  cijfer.ariaLabel.replace(
                                      /^[\s\S]*•? ?weging ([\d.,]+)[\s\S]*?$/,
                                      "$1",
                                  ),
                              )
                        ...
                        if (
                            cijfer.ariaLabel &&
                            !isNaN(weging) &&
                            !cijfer.classList.contains("gemiddelde")
                        ) {
                            subject.grades.push({
                            ...
                            if (isNaN(grade)) {
                                if (
                                    cijfer.innerText.indexOf("+") != -1 ||
                                    cijfer.innerText.indexOf("-") != -1
                                ) {
                                    subjectGradingSystems.plusmin = true;
                                } else if (
                                    cijfer.innerText.indexOf("A") != -1 ||
                                    cijfer.innerText.indexOf("B") != -1 ||
                                    cijfer.innerText.indexOf("N") != -1 ||
                                    cijfer.innerText.indexOf("L") != -1
                                ) {
                ...
                if (totalAverageWeight != 0) {
                    totalAverage = (
                        totalAverageGrades / totalAverageWeight
                    ).toFixed(2);
                }
                ...
                if (gradedata.length == 0) {
                    modMessage(
                        "Geen data",
                        "Het lijkt erop dat je nog geen cijfers hebt gekregen dit jaar. Probeer het later nog eens.",
                        "Doorgaan",
                    );
                    id("mod-message-action1").addEventListener("click", () => {
                        window.location.href =
        ...
        function orderSubjects() {
            ...
            for (var i = 0; i < 5 && i < gradedata.length; i++) {
                let randomSubject =
                    gradedata[Math.floor(Math.random() * gradedata.length)];
                if (
                    usedSubjects.includes(randomSubject.name) ||
                    !randomSubject.systems.cijfers
                ) {
                    for (const subject of gradedata) {
                        if (
                            !usedSubjects.includes(subject.name) &&
                            subject.systems.cijfers
                        ) {
                ...
                html +=
                    ...
                    randomSubject.name +
                    "</p>" +
                    window.getIcon("grip-vertical", null, "var(--text-weak)") +
                    "</div>";
            }
            cn("recap-page", 0).innerHTML =
                '<h1>Wat zijn je beste vakken?</h1><h2>Sorteer je vakken op basis van je gemiddelde</h2><div id="mod-grade-average-sort-list">' +
                html +
                '</div><a id="recap-nextpage">Volgende</a>';
            ...
            const mouseDownHandler = (e) => {
                if (e.target.classList.contains("mod-item")) {
                    ...
                    e.target.parentElement.classList.contains("mod-item")
                ) {
            ...
            [].slice
                .call(list.querySelectorAll(".mod-item"))
                .forEach((item) => {
            ...
            id("recap-nextpage").addEventListener("click", () => {
                if (clicked) {
                ...
                function checkAnswers(first = false) {
                    ...
                    for (var i = 0; i < loopCount; i++) {
                        ...
                        // Correct
                        const pElement = id("mod-grade-average-sort-list")
                            .getElementsByClassName("mod-item")
                            [i].getElementsByTagName("p")[0];
                        if (
                            pElement.innerText ==
                            orderedSubjects[maxGradeIndex].name
                        ) {
                            ...
                            if (first) {
                                pElement.insertAdjacentHTML(
                                    "afterend",
                                    "<p>" +
                                        orderedSubjects[maxGradeIndex].average
                                            .toString()
                                            .replace(".", ",") +
                                        "</p>",
                                );
            ...
            switch (pages) {
                ...
                case 6:
                    ...
                    if (availablePages.length > 0) {
                        i = Math.round(
                            Math.random() * (availablePages.length - 1),
                        );
                        availablePages[i]();
        ...
        function finish() {
            ...
            cn("recap-page", 0).innerHTML =
                '<h1>Gefeliciteerd!</h1><h2>Het jaar zit erop en de vakantie is al in zicht.</h2><h3>Veel plezier in de vakantie en hopelijk tot volgend jaar!</h3><a id="recap-nextpage">Sluiten</a>';
            id("recap-nextpage").addEventListener("click", () => {
        ...
        function award() {
            ...
            while (award == "none" && times < 50) {
                ...
                switch (randomAwardTry) {
                    ...
                    case 2:
                        if (gradingSystems.cijfers) {
                            ...
                            if (max - min < 2 && max - min > 0) {
                                award = "steady";
                                description =
                                    "het verschil tussen je hoogste en laagste punt minder dan 2 is";
                    ...
                    case 3:
                        ...
                        for (const subject of gradedata) {
                            if (
                                subject.name.toLowerCase().indexOf("wis") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("math") != -1
                            ) {
                                if (subject.average >= 5.5) {
                                    award = "algebra";
                                    description =
                                        "je een voldoende staat voor wiskunde";
                                    icon = "calculator";
                                }
                                break;
                            }
                    ...
                    case 4:
                        ...
                        for (const subject of gradedata) {
                            if (
                                subject.name.toLowerCase().indexOf("sch") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("chem") != -1
                            ) {
                                if (subject.average >= 5.5) {
                                    award = "chemicus";
                                    description =
                                        "je een voldoende staat voor scheikunde";
                                    icon = "flask";
                                }
                                break;
                            }
                    ...
                    case 5:
                        ...
                        for (const subject of gradedata) {
                            if (
                                subject.name.toLowerCase().indexOf("fra") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("eng") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("dui") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("spa") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("eng") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("chi") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("taal") != -1
                            ) {
                                if (subject.average >= 5.5) {
                                    award = "taalgenie";
                                    description =
                                        "je een voldoende staat voor een taal";
                                    icon = "earth-europe";
                                }
                                break;
                            }
                    ...
                    case 6:
                        ...
                        for (const subject of gradedata) {
                            if (
                                subject.name.toLowerCase().indexOf("gri") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("gre") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("lat") != -1
                            ) {
                                if (subject.average >= 5.5) {
                                    award = "oudheid";
                                    description =
                                        "je een voldoende staat voor een klassieke taal";
                                    icon = "landmark";
                                }
                                break;
                            }
                    ...
                    case 7:
                        ...
                        for (const subject of gradedata) {
                            if (
                                subject.name.toLowerCase().indexOf("nat") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("phy") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("bio") != -1
                            ) {
                                if (subject.average >= 5.5) {
                                    award = "wetenschapper";
                                    description =
                                        "je een voldoende staat voor natuurkunde of biologie";
                                    icon = "microscope";
                                }
                                break;
                            }
                    ...
                    case 8:
                        ...
                        for (const subject of gradedata) {
                            if (
                                subject.name.toLowerCase().indexOf("aard") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("geo") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("ges") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("his") != -1
                            ) {
                                if (subject.average >= 5.5) {
                                    award = "ontdekker";
                                    description =
                                        "je een voldoende staat voor aardrijkskunde of geschiedenis";
                    ...
                    case 9:
                        ...
                        for (const subject of gradedata) {
                            if (
                                !isNaN(parseFloat(subject.average)) &&
                                parseFloat(subject[0]) >= 8.5
                            ) {
                                max = true;
                            }
                        }
                        if (max) {
                            award = "gefocust";
                            description =
                                "je afgeronde gemiddelde van minstens &eacute;&eacute;n vak een 9 of hoger is";
                            icon = "bullseye";
                        }
                        break;
                    case 10:
                        ...
                        for (const subject of gradedata) {
                            if (
                                subject.name
                                    .toLowerCase()
                                    .indexOf("beeldend") != -1 ||
                                subject.name.toLowerCase().indexOf("kunst") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("art") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("vorming") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("ckv") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("kcv") !=
                                    -1 ||
                                subject.name.toLowerCase().indexOf("cultuur") !=
                                    -1 ||
                                subject.name
                                    .toLowerCase()
                                    .indexOf("culturele") != -1
                    ...
                    case 12: {
                        ...
                        if (gradingSystems.cijfers) {
                            ...
                            if (tens >= 3) {
                                award = "smart";
                                description =
                                    "je 3 keer een tien hebt gehaald dit jaar";
                                icon = "brain";
                            }
                        }
                        break;
            ...
            cn("recap-page", 0).innerHTML =
                ...
                window.getIcon(icon, null, "#1f86f6") +
                "</div><h1>AWARD!</h1><h2>Je hebt het dit jaar weer geweldig gedaan.</h2><h3>Omdat " +
                description +
                " krijg je de " +
                award +
                '-award.</h3><a id="recap-nextpage">Volgende</a>';
            id("recap-nextpage").addEventListener("click", closeRecapPage);
            setTimeout(() => {
                try {
        ...
        function busyYear() {
            if (totalGrades > 15) {
                cn("recap-page", 0).innerHTML =
                    "<h1>" +
                    (pages > 1 ? "En w" : "W") +
                    "at was het toch een druk jaar 😅</h1><h2>Je hebt in totaal <i>" +
                    totalGrades +
                    "</i> " +
                    (totalGrades == 1 ? "cijfer" : "cijfers") +
                    " gekregen met een totale weging van maar liefst <i>" +
                    totalWeight +
                    '</i>!</h2><h3>Dat is wel een applausje waard!</h3><a id="recap-nextpage">Volgende</a>';
            } else {
                cn("recap-page", 0).innerHTML =
                    "<h1>Lekker rustig jaartje 😎</h1><h2>Je hebt in totaal <i>" +
                    totalGrades +
                    "</i> " +
                    (totalGrades == 1 ? "cijfer" : "cijfers") +
                    " gekregen met een totale weging van <i>" +
                    totalWeight +
                    '</i>!</h2><h3>Dat is relaxed door het jaar heen gaan!</h3><a id="recap-nextpage">Volgende</a>';
            }
            id("recap-nextpage").addEventListener("click", closeRecapPage);
        }
        function subjectsHigh() {
            ...
            if (
                highest >= 5.5 &&
                (secondHighest == null || secondHighest >= 5.5)
            ) {
                cn("recap-page", 0).innerHTML =
                    "<h1>" +
                    (pages > 1 ? "En w" : "W") +
                    "at heb je toch goede cijfers gehaald </h1><h2>Toen je een <i>" +
                    highest +
                    "</i> haalde voor " +
                    highestName +
                    " was je echt de uitblinker van de klas ✨</h2>" +
                    (secondHighest
                        ? "<h3>En vergeet ook niet de <i>" +
                          secondHighest +
                          "</i> die je voor " +
                          secondHighestName +
                          " haalde!</h3>"
                        : "") +
                    '<a id="recap-nextpage">Volgende</a>';
            } else {
                cn("recap-page", 0).innerHTML =
                    "<h1>Laten we naar je cijfers kijken</h1><h2>Daar is wel wat ruimte voor verbetering 😅. Je hoogste cijfer is een <i>" +
                    highest +
                    "</i> voor " +
                    highestName +
                    ".</h2>" +
                    (secondHighest
                        ? "<h3>En je op een na hoogste een <i>" +
                          secondHighest +
                          "</i> die je voor " +
                          secondHighestName +
                          " kreeg.</h3>"
        ...
        function subjectsLow() {
            ...
            if (lowest < 5.5) {
                cn("recap-page", 0).innerHTML =
                    "<h1>Soms was een toets vervelend 😓</h1><h2>Sommige toetsen zijn veel te moeilijk. Zoals de toets waarbij je een <i>" +
                    lowest +
                    "</i> haalde voor " +
                    lowestName +
                    ".</h2><h3>" +
                    (lowestAverage < 5.5
                        ? "Toch heb je dit nog wel wat omhoog weten te halen naar een <i>" +
                          lowestAverage +
                          "</i>."
                        : "Gelukkig bleef je doorzetten en sta je nu toch voldoende met een gemiddelde van <i>" +
                          lowestAverage +
                          "</i>!") +
                    '</h3><a id="recap-nextpage">Volgende</a>';
            } else {
                cn("recap-page", 0).innerHTML =
                    "<h1>Wow, alleen maar voldoendes!</h1><h2>Je hebt geen enkele onvoldoende gekregen dit jaar 🥳! Je laagste cijfer was een <i>" +
                    lowest +
                    "</i> voor " +
                    lowestName +
                    ".</h2><h3>Ondanks dat " +
                    lowestName +
                    " soms lastig is, is je gemiddelde voor dit vak een <i>" +
                    lowestAverage +
                    '</i>!</h3><a id="recap-nextpage">Volgende</a>';
            }
            id("recap-nextpage").addEventListener("click", closeRecapPage);
        }
        function twoTrueOneFalse() {
            ...
            const averageSubject =
                gradedata[Math.round(Math.random() * (gradedata.length - 1))];
            ...
            let grade =
                gradeSubject.grades[
                    Math.round(Math.random() * (gradeSubject.grades.length - 1))
                ].cijfer;
            // amount
            const amountSubject =
                gradedata[Math.round(Math.random() * (gradedata.length - 1))];
            ...
            switch (random) {
                case 1:
                    ...
                    if (isNaN(parseFloat(average))) {
                        ...
                        const add =
                            Math.round(Math.random() * (2 - 0.5) + 0.5 * 100) /
                            100;
                ...
                case 2:
                    ...
                    if (gradeSubject.systems.cijfers) {
                        ...
                        while (true) {
                            grade +=
                                Math.round(Math.random() * (1 + 1) - 1 * 10) /
                                10;
            ...
            cn("recap-page", 0).innerHTML =
                '<h1>Kies een van de opties.</h1><h2>Welke van de volgende opties klopt niet? 🤔</h2><label><input type="checkbox" id="recap-option-1"/><p>' +
                (replacementSubject
                    ? "Je cijfers voor " +
                      replacementSubject.name +
                      ' hebben een totale weging van <span class="number">' +
                      replacement +
                      '</span><span class="correction"></span>'
                    : 'Je staat een <span class="number">' +
                      average +
                      '</span><span class="correction"></span> voor ' +
                      averageSubject.name) +
                '</p></label><label><input type="checkbox" id="recap-option-2"/><p>Je hebt een <span class="number">' +
                grade +
                '</span><span class="correction"></span> voor ' +
                gradeSubject.name +
                ' gehaald</p></label><label><input type="checkbox" id="recap-option-3"/><p>Je hebt <span class="number">' +
                amount +
                '</span><span class="correction"></span> ' +
                (amount == 1 ? "cijfer" : "cijfers") +
                " voor " +
                amountSubject.name +
                ' gehaald</p></label><a id="recap-nextpage">Controleren</a>';
            let clicked = false;
            id("recap-nextpage").addEventListener("click", function () {
                ...
                if (
                    !id("recap-option-1").checked &&
                    !id("recap-option-2").checked &&
                    !id("recap-option-3").checked
                ) {
                    return;
                }
                ...
                if (id("recap-option-" + random).checked) {
                    clicked = true;
                    id("recap-option-" + random).parentElement.classList.add(
                        "right",
                    );
                    id(
                        "recap-option-" + random,
                    ...
                    if (id("recap-option-1").checked) {
                        id("recap-option-1").parentElement.classList.add(
                            "wrong",
                        );
                    }
                    if (id("recap-option-2").checked) {
                        id("recap-option-2").parentElement.classList.add(
                            "wrong",
                        );
                    }
                    if (id("recap-option-3").checked) {
                        id("recap-option-3").parentElement.classList.add(
                            "wrong",
                        );
                    }
                    id("recap-option-" + random).parentElement.classList.add(
            ...
            id("recap-option-1").addEventListener("change", () => {
                id("recap-option-2").checked = false;
                id("recap-option-3").checked = false;
            });
            id("recap-option-2").addEventListener("change", () => {
                id("recap-option-1").checked = false;
                id("recap-option-3").checked = false;
            });
            id("recap-option-3").addEventListener("change", () => {
                id("recap-option-1").checked = false;
                id("recap-option-2").checked = false;
            });
        }
        function overgangsCheck() {
            ...
            let tekorten = 0,
                compensatie = 0;
            const kernvakken_onvoldoende = [];
            const kernvakken_5_of_hoger = [];
            const alle_vakken_4_of_hoger = [];
            const onvoldoendes = [];
            let overgang = 100;
            for (const subject of gradedata) {
                if (!isNaN(parseFloat(subject.average))) {
                    ...
                    const kernvak =
                        subject.name.toLowerCase().indexOf("wiskunde") != -1 &&
                        subject.name.toLowerCase().indexOf("nederlands") !=
                            -1 &&
                        subject.name.toLowerCase().indexOf("engels") != -1;
            ...
            cn("recap-page", 0).innerHTML =
                "<h1>" +
                overgang +
                "% kans om over te gaan</h1><h2>" +
                (overgang == 100
                    ? "Met deze flawless cijferlijst ga je natuurlijk zeker weten over! ✅"
                    : overgang > 75
                      ? "Prima gedaan! Af en toe een onvoldoende halen kan gebeuren, maar dat zit jou nooit in de weg! 🙃"
                      : overgang > 50
                        ? "Redelijke cijferlijst, al voldoe je niet aan alle overgangsnormen."
                        : overgang >= 25
                          ? "Hmm, dat gaat een taai overgangsgesprek worden. Maar jij kan dit! Veel succes!"
                          : "Hmm, dat gaat een taai overgangsgesprek worden. Hopelijk komt het goed voor je.") +
                '</h3><br><br><div id="recap-chart-wrapper"><canvas id="recap-chart" width="350" height="350"></canvas></div><a id="recap-nextpage">Doorgaan</a>';
            id("recap-nextpage").addEventListener("click", closeRecapPage);
            if (
                distribution[9] +
                    distribution[8] +
            ...
            const recapCanvas = document.getElementById("recap-chart");
            const recapCtx = recapCanvas.getContext("2d");
            const recapChart = new Chart(recapCtx, {
                type: "doughnut",
                data: {
        ...
        function onvoldoendeGraph() {
            ...
            const percentage =
                (Math.max(voldoende, 1) / Math.max(onvoldoende, 1)) * 100;
            cn("recap-page", 0).innerHTML =
                "<h1>" +
                (percentage > 85
                    ? "Voldoende?!? JA!"
                ...
                "</h1><h2>" +
                (percentage > 85
                    ? "Natuurlijk heb je dit jaar weer veel voldoendes gehaald!"
                    : percentage > 50
                      ? "Jazeker! Je hebt meer dan de helft voldoende!"
                      : "Dat is niet zo best. Volgend jaar beter?") +
                '</h3><br><br><div id="recap-chart-wrapper"><canvas id="recap-chart" width="300" height="300"></canvas></div><a id="recap-nextpage">Doorgaan</a>';
            Chart.defaults.color = "#fff";
            const recapCanvas = document.getElementById("recap-chart");
            const recapCtx = recapCanvas.getContext("2d");
            const recapChart = new Chart(recapCtx, {
        ...
        function completedGraph() {
            ...
            cn("recap-page", 0).innerHTML =
                '<h1>Doel gehaald! 🎯</h1><h2>Goed gedaan! Ook dit jaar is weer voorbij!</h2><h3>Hoeveel proefwerken en opdrachten heb je wel niet gemaakt? Veel.</h3><br><br><div id="recap-chart-wrapper"><canvas id="recap-chart" width="300" height="300"></canvas></div><a id="recap-nextpage">Doorgaan</a>';
            Chart.defaults.color = "#fff";
            const recapCanvas = document.getElementById("recap-chart");
            const recapCtx = recapCanvas.getContext("2d");
            const recapChart = new Chart(recapCtx, {
        ...
        function plusMinGraph() {
            ...
            for (const subject of gradedata) {
                if (subject.systems.plusmin) {
                    for (const grade of subject.grades) {
                        const containsPlus =
                            grade.cijfer.toString().indexOf("+") != -1;
                        const containsMin =
                            grade.cijfer.toString().indexOf("-") != -1;
            ...
            cn("recap-page", 0).innerHTML =
                "<h1>Je krijgt een dikke plus!</h1><h2>" +
                (plus > min
                    ? "Ook dit jaar zijn er weer flink wat plussen uitgedeeld!"
                    : plus < min
                      ? "De minnen overtroffen dit jaar helaas de plussen... Maar niet getreurd, want je hebt toch nog " +
                        plus +
                        " plussen bij elkaar weten te verzamelen! Goed gedaan!"
                      : "Het is een gelijkspel! Je hebt precies evenveel plussen als minnen!") +
                '</h2><br><br><div id="recap-chart-wrapper"><canvas id="recap-chart"></canvas></div><a id="recap-nextpage">Doorgaan</a>';
            Chart.defaults.color = "#fff";
            const recapCanvas = document.getElementById("recap-chart");
            const recapCtx = recapCanvas.getContext("2d");
            const recapChart = new Chart(recapCtx, {
        ...
        function guessTheGraph() {
            ...
            while (fakeOptionOne == chosenSubject.name) {
                if (i > 100) {
                    fakeOptionOne =
                        subjectArray[
                            Math.floor(Math.random() * subjectArray.length)
                        ];
                    break;
                }
                fakeOptionOne =
            ...
            let fakeOptionTwo =
                gradedata[Math.round(Math.random() * (gradedata.length - 1))]
                    .name;
            while (
                fakeOptionTwo == chosenSubject.name ||
                fakeOptionTwo == fakeOptionOne
            ) {
                if (i > 100) {
                    fakeOptionTwo =
                        subjectArray[
    ...
    function openSettings() {
        ...
        if (tn("sl-account-modal", 0)) {
            setTimeout(() => {
                ...
                if (
                    tn("sl-account-modal-header", 1) &&
                    tn("sl-account-modal-header", 1).getElementsByClassName(
                        "title ng-star-inserted",
                    )[0]
                ) {
                    tn("sl-account-modal-header", 1).getElementsByClassName(
                        "title ng-star-inserted",
                    )[0].dataset.originalText = tn(
                        "sl-account-modal-header",
                        1,
                    ).getElementsByClassName(
                        "title ng-star-inserted",
                    )[0].innerHTML;
                ...
                if (
                    ...
                    tn("sl-account-modal", 0).getElementsByClassName(
                        "container",
                    )[0]
                ) {
                    if (
                        !tn("sl-account-modal", 0)
                            .getElementsByClassName("container")[0]
                            .classList.contains("show-details")
                    ) {
                        if (
                            tn("sl-account-modal-tab", 0).classList.contains(
                                "active",
                            )
                        ) {
            ...
            if (
                ...
                tn("sl-account-modal", 0).insertAdjacentHTML(
                    "beforeend",
                    '<div class="content" style="padding: 20px 40px;"></div>',
                );
            }
            if (
                tn("sl-account-modal", 0).getElementsByClassName(
                ...
                tn("sl-account-modal", 0).getElementsByClassName("content")[0]
                    .children[0] &&
                tn("sl-account-modal", 0).getElementsByClassName("content")[0]
                ...
                tn("sl-account-modal", 0).getElementsByClassName(
                    "content",
                )[0].children[0].children[0].inert = true;
            }
            if (
                tn("sl-account-modal", 0).getElementsByClassName(
                ...
                tn("sl-account-modal", 0)
                    .getElementsByClassName("ng-star-inserted active")[0]
                    .classList.remove("active");
            }
            if (
                n(
                ...
                if (
                    tn("sl-account-modal-tab", 0).classList.contains("active")
                ) {
                    tn("sl-account-modal-tab", 1).click();
                } else {
                    tn("sl-account-modal-tab", 0).click();
            ...
            if (Array.isArray(nicknameArray) == false) {
                nicknameArray = [];
                set("nicknames", "[]");
            }
            nicknameArray.push(["", "", ""]);
            for (const nickname of nicknameArray) {
                if (nickname.length == 2 || nickname.length == 3) {
                    ...
                    const teacherAbbreviation = sanitizeString(
                        nickname[2] ?? "",
                    );
            ...
            // Update details
            const updateinfo = isExtension
                ? "Je browser controleert automatisch op updates."
                : "Je userscriptmanager controleert automatisch op updates.";
            ...
            const openDyslexicEnabled = tn("span", 0)
                ? window
                      .getComputedStyle(tn("span", 0))
                      .getPropertyValue("font-family")
                      .indexOf("OpenDyslexic") != -1
                : false;
            ...
            const avatarHiddenOverridden =
                document.querySelector(".avatar .initials") &&
                !n(get("profilepic")) &&
                n(document.getElementsByClassName("profielfoto-optie")[0]);
            const weergave =
            ...
            const ngDetected = /(_ngcontent|_nghost|ng-tns-c\d+|ng-c\d+)/.test(
                get("customcss") ?? "",
            );
            const night = document
                .getElementsByTagName("html")[0]
                .classList.contains("night");

            // Fetch settings template HTML
            let settingsContent =
                window.getResourceAsText("data/settings.html");
            if (n(settingsContent)) {
                settingsContent =
                    '<h3 style="margin-top: 200px;">Error</h3><p style="margin-bottom: 200px;">Could not generate the settings HTML at this moment. Try again later.</h3>';
            }

            // Replace keys in replacement with real content
            const replacements = {
                "{{icon_floppy_disk}}": window.getIcon(
                    "floppy-disk",
                    "mod-save-shake",
                    "var(--text-moderate)",
                ),
                "{{icon_rotate_left}}": window.getIcon(
                    "rotate-left",
                    "mod-reset-rotate",
                    "var(--text-moderate)",
                ),
                "{{icon_circle_info}}": window.getIcon(
                    "circle-info",
                    "mod-info-wobble",
                    "var(--text-moderate)",
                ),
                "{{icon_circle_exclamation}}": window.getIcon(
                    "circle-exclamation",
                    "mod-bug-scale",
                    "var(--text-moderate)",
                ),
                "{{icon_upload}}": window.getIcon(
                    "upload",
                    null,
                    "var(--fg-on-primary-weak)",
                ),
                "{{icon_shuffle}}": window.getIcon(
                    "shuffle",
                    null,
                    "var(--fg-on-primary-weak)",
                ),
                "{{icon_palette}}": window.getIcon(
                    "palette",
                    null,
                    "var(--fg-on-primary-weak)",
                ),
                "{{icon_edit}}": window.getIcon(
                    "edit",
                    null,
                    "var(--fg-on-primary-weak)",
                ),
                "{{addSetting_primarycolor}}": addSetting(
                    "Primaire kleur",
                    null,
                    "primarycolor",
                    "color",
                ...
                "{{addSetting_secondarycolor}}": addSetting(
                    "Secundaire kleur",
                    null,
                    "secondarycolor",
                    "color",
                ...
                "{{addLiveColors}}":
                    addSetting(null, null, "livecolor1", "color", "#000000") +
                    addSetting(null, null, "livecolor2", "color", "#000000") +
                    addSetting(null, null, "livecolor3", "color", "#000000"),
                "{{backgroundtype_image_active}}":
                    n(get("backgroundtype")) || get("backgroundtype") == "image"
                        ? "active"
                        : "",
                "{{backgroundtype_slideshow_active}}":
                    get("backgroundtype") == "slideshow" ? "active" : "",
                "{{backgroundtype_color_active}}":
                    get("backgroundtype") == "color" ? "active" : "",
                "{{backgroundtype_live_active}}":
                    get("backgroundtype") == "live" ? "active" : "",
                "{{display_bg_image}}":
                    n(get("backgroundtype")) || get("backgroundtype") == "image"
                        ? "block"
                        : "none",
                "{{display_mod_filters}}": n(get("background"))
                    ? "display:none;"
                    : "",
                "{{video_style}}": n(id("mod-background"))
                    ? ""
                    : "filter:" + id("mod-background").style.filter + ";",
                "{{video_src}}":
                    get("isbackgroundvideo") &&
                    get("isbackgroundvideo") != "false"
                        ? `src="${get("background")}"`
                ...
                "{{addSetting_background}}": addSetting(
                    ...
                    "image/*, video/*",
                    null,
                    `<div tabindex="0" class="mod-button" id="mod-random-background">${window.getIcon("shuffle", null, "var(--fg-on-primary-weak)")}<span>Random</span></div>`,
                ),
                "{{display_bg_slideshow}}":
                    get("backgroundtype") == "slideshow" ? "block" : "none",
                "{{display_bg_color}}":
                    get("backgroundtype") == "color" ? "block" : "none",
                "{{display_bg_live}}":
                    get("backgroundtype") == "live" ? "block" : "none",
                "{{addSetting_backgroundcolor}}": addSetting(
                    "Achtergrondkleur",
                    null,
                    "backgroundcolor",
                    "color",
                    darkmode ? "#20262d" : "#ffffff",
                ),
                "{{addSetting_ui_transparency}}": night
                    ? '<div class="br"></div><div class="mod-info-notice">' +
                      window.getIcon(
                          "circle-info",
                          null,
                          "var(--fg-on-primary-weak)",
                      ...
                      "Somtoday Mod Night mode ondersteunt momenteel geen UI transparantie en/of blur.</div>"
                    : addSetting(
                          "UI-transparantie",
                          "Verander de transparantie van de UI.",
                          "ui",
                          "range",
                          get("ui"),
                          0,
                ...
                "{{addSetting_ui_blur}}": night
                    ? ""
                    : addSetting(
                          "UI-blur",
                          "Verander de blur van de UI.",
                          "uiblur",
                          "range",
                          get("uiblur"),
                          0,
                ...
                "{{layout_1}}":
                    '<div tabindex="0" class="layout-container' +
                    (get("layout") == 1 ? " layout-selected" : "") +
                    '" id="layout-1"><div style="width:94%;height:19%;top:4%;left: 3%;"></div><div style="width:94%;height:68%;top:27%;left:3%;"></div><h3>Standaard</h3></div>',
                "{{layout_2}}":
                    '<div tabindex="0" class="layout-container' +
                    (get("layout") == 2 ? " layout-selected" : "") +
                    '" id="layout-2"><div style="width: 16%; height: 92%; top: 4%; left: 3%;"></div><div style="width: 75%; height: 92%; right: 3%; top: 4%;"></div><h3>Sidebar links</h3></div>',
                "{{layout_3}}":
                    '<div tabindex="0" class="layout-container' +
                    (get("layout") == 3 ? " layout-selected" : "") +
                    '" id="layout-3"><div style="width:75%;height:92%;left:3%;top:4%;"></div><div style="width:16%;height:92%;right:3%;top:4%;"></div><h3>Sidebar rechts</h3></div>',
                "{{layout_4}}":
                    '<div tabindex="0" class="layout-container' +
                    (get("layout") == 4 ? " layout-selected" : "") +
                    '" id="layout-4"><div style="width:68%;height:19%;top:4%;left:16%;"></div><div style="width: 68%;height:68%;top:27%;left: 16%;"></div><h3>Gecentreerd</h3></div>',
                "{{layout_5}}":
                    '<div tabindex="0" class="layout-container' +
                    (get("layout") == 5 ? " layout-selected" : "") +
                    '" id="layout-5"><div style="width:16%;height:92%;top:4%;left:3%;"></div><div style="width:75%;height:19%;right:3%;top:4%;"></div><div style="width:75%;height:69%;right:3%;top:27%;"></div><h3>Menu & sidebar</h3></div>',
                "{{menu_settings}}":
                    addSetting(
                        "Laat menu altijd zien",
                        "Toon de bovenste menubalk altijd. Als dit uitstaat, verdwijnt deze als je naar beneden scrolt.",
                        "bools00",
                        "checkbox",
                        true,
                    ) +
                    addSetting(
                        "Paginanaam in menu",
                        "Laat een tekst met de paginanaam zien in het menu.",
                        "bools01",
                        "checkbox",
                        true,
                    ) +
                    addSetting(
                        "Verberg bericht teller",
                        "Verberg het tellertje dat het aantal ongelezen berichten aangeeft.",
                        "bools02",
                        "checkbox",
                        false,
                    ),
                "{{nicknames}}": nicknames,
                "{{username_wrapper}}":
                    '<h3>Gebruikersnaam</h3><p>Verander je gebruikersnaam.</p><div id="username-wrapper"><div><input title="Echte naam" class="mod-custom-setting" id="realname" type="text" placeholder="Echte naam" value="' +
                    (n(get("realname")) ? "" : get("realname")) +
                    '"><input title="Nieuwe gebruikersnaam" class="mod-custom-setting" id="username" type="text" placeholder="Nieuwe gebruikersnaam" value="' +
                    (n(get("username"))
                        ? ""
                        : sanitizeString(get("username"))) +
                    '"></div></div>',
                "{{font_settings}}":
                    `
                    <h3>Lettertype</h3>` +
                    (openDyslexicEnabled
                        ? '<div class="br"></div><div class="mod-info-notice">' +
                          ...
                          "De instelling <b>" +
                          weergave +
                          'Weergave > Optimaliseer voor dyslexie</b> moet uitstaan om dit te laten werken.</div><div class="br"></div><div class="br"></div>'
                        : "") +
                    `
                    <div class="br"></div>
                    <div style="display: flex; flex-wrap: wrap; gap: 6px; align-items: center;">
                ...
                "{{profilepic_setting}}": addSetting(
                    "Profielafbeelding",
                    "Gebruik een eigen profielafbeelding in plaats van je schoolfoto." +
                        (avatarHiddenOverridden
                            ? '<div class="mod-info-notice">' +
                              ...
                              "De instelling <b>" +
                              weergave +
                              "Weergave > Verberg profielfoto</b> wordt genegeerd omdat je een custom profielafbeelding hebt ingesteld. Reset deze instelling om je initialen te tonen.</div>"
                            : ""),
                    "profilepic",
                    "file",
                    null,
                ...
                "{{grade_reveal_setting}}":
                    '<div><h3>Cijfer-reveal</h3><p style="margin-right:15px;">Toon bij je cijfers een optel-animatie.</p><div id="grade-reveal-select" class="mod-multi-choice"><span' +
                    (get("bools").charAt(BOOL_INDEX.GRADE_REVEAL) == "1"
                        ? ' class="active"'
                        : "") +
                    ' tabindex="0">Alleen bij nieuwe cijfers</span><span' +
                    (get("bools").charAt(BOOL_INDEX.GRADE_REVEAL) == "2"
                        ? ' class="active"'
                        : "") +
                    ' tabindex="0">Altijd</span><span' +
                    (get("bools").charAt(BOOL_INDEX.GRADE_REVEAL) == "0"
                        ? ' class="active"'
                        : "") +
                    ' tabindex="0">Nooit</span></div></div>',
                ...
                "{{extra_settings}}":
                    addSetting(
                        "Analyse op cijferpagina",
                        "Laat een korte analyse zien op de cijfer-pagina van een vak.",
                        "bools18",
                        "checkbox",
                        true,
                    ) +
                    addSetting(
                        "Compact rooster",
                        "Maak je rooster compacter door lesuren in een grid te zetten. Werkt niet voor alle scholen.",
                        "bools03",
                        "checkbox",
                        false,
                    ) +
                    addSetting(
                        "Deel debug-data",
                        "Verstuur bij een error anonieme informatie naar de developers om Somtoday Mod te verbeteren.",
                        "bools04",
                        "checkbox",
                        false,
                    ) +
                    addSetting(
                        "Downloadknop voor cijfers",
                        "Laat een downloadknop zien op de laatste cijfers en vakgemiddelden-pagina.",
                        "bools05",
                        "checkbox",
                        true,
                    ) +
                    addSetting(
                        "Feestdagen",
                        "Laat bij feestdagen soms iets zien, zoals een kerstmuts op het Somtoday-logo.",
                        "bools17",
                        "checkbox",
                        true,
                    ) +
                    addSetting(
                        "Felicitatieberichten",
                        "Laat een felicitatiebericht zien als je jarig bent, of als je al een aantal jaar van Somtoday Mod gebruik maakt.",
                        "bools06",
                        "checkbox",
                        true,
                    ) +
                    addSetting(
                        "Grafieken op cijferpagina",
                        "Laat een cijfer- en gemiddeldegrafiek zien op de cijfer-pagina van een vak.",
                        "bools07",
                        "checkbox",
                        true,
                    ) +
                    ...
                    get("layout") == 5
                        ? addSetting(
                              "Logo van mod in menu",
                              "Laat in plaats van het logo van Somtoday het logo van Somtoday Mod zien.",
                              "bools08",
                              "checkbox",
                              true,
                          )
                    ...
                    addSetting(
                        "Raster bij rooster",
                        "Laat een raster zien achter je rooster.",
                        "bools15",
                        "checkbox",
                        true,
                    ) +
                    addSetting(
                        "Redirect naar ELO",
                        "Redirect je automatisch van https://som.today naar https://inloggen.somtoday.nl.",
                        "bools09",
                        "checkbox",
                        true,
                    ) +
                    addSetting(
                        "Rekentool op cijferpagina",
                        "Voeg een rekentool toe op de cijferpagina om snel te berekenen welk cijfer je moet halen.",
                        "bools10",
                        "checkbox",
                        true,
                    ) +
                    addSetting(
                        "Scrollbar",
                        "Laat de scrollbar van een pagina zien.",
                        "bools11",
                        "checkbox",
                        true,
                    ) +
                    ...
                    addSetting(
                        "Somtoday Recap",
                        "Laat aan het einde van het schooljaar een recap-knop zien (vanaf 26 juni).",
                        "bools12",
                        "checkbox",
                        true,
                    ) +
                    addSetting(
                        "Taken toevoegen",
                        "Laat een knop zien om taken toe te voegen aan de studiewijzer.",
                        "bools16",
                        "checkbox",
                        true,
                    ),
                "{{browser_settings}}":
                    addSetting(
                        "Titel",
                        "Verander de titel van Somtoday in de tabbladen van de browser.",
                        "title",
                        "text",
                        "",
                        "Somtoday",
                    ...
                    addSetting(
                        "Icoon",
                        "Verander het icoontje van Somtoday in de menubalk van de browser. Accepteert png, jpg/jpeg, gif, svg, ico en meer.</p>" +
                            (platform == "Firefox"
                                ? ""
                                : '<div class="mod-info-notice">' +
                                  window.getIcon(
                                  ...
                                  "Bewegende GIF-bestanden werken alleen in Firefox.</div>") +
                            '<div class="br"></div><p>',
                        "icon",
                        "file",
                        null,
                    ...
                    addSetting(
                        "Aangepaste CSS",
                        "Voer hier je eigen CSS in om Somtoday nóg verder te veranderen. Dit is een geavanceerde instelling voor gebruikers die weten hoe CSS werkt.",
                        "customcss",
                        "textarea",
                        "",
                        "/* Voorbeeld: */\nbody {\n    background: red;\n}",
                    ...
                    '<div id="angular-hash-warning" style="display: ' +
                    (ngDetected ? "block" : "none") +
                    ';"><div class="br"></div><div class="mod-info-notice">' +
                    window.getIcon(
                        "circle-info",
                        null,
                ...
                "{{autologin_warning}}":
                    get("logincredentialsincorrect") == "1"
                        ? '<div class="mod-info-notice">' +
                          ...
                          'Autologin is tijdelijk uitgeschakeld.</div><div class="br"></div><div class="br"></div><div class="br"></div>'
                        : "",
                "{{autologin_school}}": addSetting(
                    "School",
                    "Voer je schoolnaam in.",
                    "loginschool",
                    "text",
                    "",
                    "",
                ),
                "{{autologin_name}}": addSetting(
                    "Gebruikersnaam",
                ...
                "{{somtoday_version}}":
                    "Versie " + version_name + " van Somtoday Mod",
            ...
            tn("sl-account-modal", 0)
                .getElementsByClassName("content")[0]
                .children[0].insertAdjacentHTML("beforeend", settingsContent);

            for (const element of cn("mod-game")) {
                element.addEventListener("click", () => {
                    execute([startTheEscape]);
                });
            }
            ...
            if (
                ...
                id("import-settings").addEventListener("click", () => {
                    modMessage(
                        "Instellingen importeren?",
                        "Wanneer je Mod-instellingen importeert worden je huidige instellingen overschreven. Importeer alleen instellingsbestanden die je vertrouwt of zelf hebt ge&euml;xporteerd.",
                        "Ja",
                        "Nee",
                    );
                    id("mod-message-action1").addEventListener("click", () => {
                ...
                id("import-settings-json").addEventListener(
                    "input",
                    importSettings,
                );
            }
                ...
                id("mod-font-file").addEventListener("input", function () {
                    if (this.files.length != 0) {
                        ...
                        this.parentElement.getElementsByTagName(
                            ...
                            this.files[0].name.toLowerCase();
                        this.parentElement
                            .getElementsByTagName("label")[0]
                            .classList.add("mod-active");
                        ...
                        reader.onload = () => {
                            setTimeout(() => {
                                tn("head", 0).insertAdjacentHTML(
                                    "beforeend",
                                    '<style id="mod-font-preview">@font-face{font-family:modCustomFontPreview;src:url("' +
                                        reader.result +
                                        '");}</style>',
                                );
                                document
                                    .getElementById("font-box")
                                    .children[0].style.setProperty(
                                        "font-family",
                                        "modCustomFontPreview, sans-serif",
                                        "important",
                                    );
                                document
                                    .getElementById("font-box")
                        ...
                        this.parentElement.getElementsByTagName(
                            "label",
                        )[0].children[1].innerText = "Of upload lettertype";
                        this.parentElement
                            .getElementsByTagName("label")[0]
                            .classList.remove("mod-active");
                        this.value = null;
            ...
            if (id("customcss")) {
                ...
                id("customcss").addEventListener("keydown", (e) => {
                    if (e.key === "Tab") {
                        e.preventDefault();
                        e.preventDefault();
                        ...
                        if (
                            ...
                            id("customcss").value =
                                id("customcss").value.substring(0, start) +
                                "    " +
                                id("customcss").value.substring(end);

                            id("customcss").selectionStart = id(
                                "customcss",
                ...
                // Live input checking
                id("customcss").addEventListener("input", (e) => {
                    const content = e.target.value;

                    // Angular hash check
                    if (
                        /(_ngcontent|_nghost|ng-tns-c\d+|ng-c\d+)/.test(content)
                    ) {
            ...
            id("theme-wrapper").insertAdjacentHTML(
                "afterend",
                `<div id="more-themes" tabindex="0" class="mod-button">Meer bekijken</div>`,
            );
            document
                .getElementById("more-themes")
                .addEventListener("click", function () {
                    const hiddenThemes =
                        this.previousElementSibling.querySelectorAll(
                            ".theme[style]",
                        );
                    for (let i = 0; i < 10; i++) {
            ...
            async function addImage(url, element = null) {
                return new Promise((resolve, reject) => {
                    ...
                    img.addEventListener("click", function () {
                        id("mod-background-wrapper").classList.add(
                            "mod-modified",
                        );
                        this.remove();
                    });
            ...
            if (
                id("mod-background-preview-image") &&
                id("mod-background-preview-video")
            ) {
                const isbackgroundvideo =
                    get("isbackgroundvideo") &&
                    get("isbackgroundvideo") != "false";
                id("mod-background-preview-image").style.setProperty(
                    "filter",
                    getBackgroundFilters(false),
                );
                id("mod-background-preview-video").style.setProperty(
                    "filter",
                    getBackgroundFilters(false),
                );
                id("mod-background-preview-image").style.display =
                    !isbackgroundvideo ? "block" : "none";
                id("mod-background-preview-video").style.display =
                    isbackgroundvideo ? "block" : "none";
            }
            if (id("addbackground")) {
            ...
            if (id("background")) {
                id("background").addEventListener("input", function () {
                    if (this.files[0] != null) {
                        const isVideo =
                            this.files[0].type.indexOf("video") != -1;
                        const reader = new FileReader();
                        ...
                        reader.onload = () => {
                            if (isVideo) {
                                id("mod-background-preview-video").src =
                                    reader.result;
                            } else {
                                id("mod-background-preview-image").src =
                                    reader.result;
                            }
                        };
                    } else {
            ...
            if (id("mod-reset-filters")) {
                id("mod-reset-filters").addEventListener("click", () => {
            ...
            for (const element of cn("mod-file-label")) {
                element.addEventListener("drop", (event) => {
                    ...
                    if (event.dataTransfer.items) {
                        [...event.dataTransfer.items].forEach((item, i) => {
                            if (!done && item.kind === "file") {
                                ...
                                element.parentElement.previousElementSibling.files =
                                    fileList;
                            }
                        });
                    } else {
                    ...
                    element.parentElement.previousElementSibling.dispatchEvent(
                        inputEvent,
                    );
                });
            ...
            for (const element of cn("mod-file-reset")) {
                element.addEventListener("click", () => {
                    const shouldBeActive =
                        !element.classList.contains("mod-active");
                    for (const btn of element.parentElement.getElementsByClassName(
                        "mod-active",
                    )) {
            ...
            for (const element of id(
                ...
                const index = number;
                id("category-" + element.dataset.category).style.display =
                    categories.charAt(index) == "1" ? "block" : "none";
                ...
                element.addEventListener("click", () => {
                    ...
                    const collapsed =
                        id("category-" + element.dataset.category).style
                            .display == "none";
                    set(
                        "category",
                        get("category").replaceAt(index, collapsed ? "1" : "0"),
                    );
                    if (collapsed) {
                        element.classList.remove("collapsed");
                    } else {
            ...
            if (id("reset")) {
                id("reset").addEventListener("click", () => {
                    modMessage(
                        "Alles resetten?",
                        "Al je instellingen zullen worden gereset. Weet je zeker dat je door wil gaan?",
                        "Ja",
                        "Nee",
                    );
                    id("mod-message-action1").addEventListener("click", () => {
                        ...
                        if (
                            id("mod-grades-graphs") &&
                            get("bools").charAt(BOOL_INDEX.SUBJECT_GRAPHS) ==
                                "1" &&
                            tn("sl-vakresultaten", 0)
                        ) {
                            tryRemove(id("mod-grades-graphs"));
                            tn("sl-vakresultaten", 0).insertAdjacentHTML(
                                "beforeend",
                                '<div id="mod-grades-graphs"><h3>Mijn cijfers</h3><div><canvas id="mod-chart-1"></canvas></div><h3>Mijn gemiddelde</h3><div><canvas id="mod-chart-2"></canvas></div></div>',
                            );
            ...
            if (id("mod-live-random")) {
                ...
                id("primarycolor").addEventListener("change", () => {
                    if (liveType == "primary") {
                        id("mod-live-primary").click();
                    }
                });

                id("secondarycolor").addEventListener("change", () => {
                ...
                id("mod-live-random").addEventListener("click", () => {
                    liveType = "random";
                    col1 = randomColor(0);
                    col2 = randomColor(2);
                    col3 = randomColor(4);
                ...
                id("mod-live-primary").addEventListener("click", () => {
                ...
                id("mod-live-secondary").addEventListener("click", () => {
                ...
                id("mod-live-custom").addEventListener("click", () => {
                    liveType = "custom";
            ...
            if (id("mod-change-letterbeoordelingen")) {
                id("mod-change-letterbeoordelingen").addEventListener(
                    "click",
                    () => {
    ...
    function closeSettings(element) {
        ...
        if (tn("sl-account-modal", 0)) {
            if (
                ...
                tn("sl-account-modal-header", 1).getElementsByClassName(
                    "title ng-star-inserted",
                )[0].dataset.originalText;
            }
            tn("sl-account-modal-header", 1).getElementsByClassName(
                "title ng-star-inserted",
            )[0].innerHTML = element.getElementsByTagName("span")[0].innerHTML;
            if (
                tn("sl-account-modal", 0).getElementsByClassName("content")[0]
            ) {
                if (
                    ...
                    if (
                        ...
                        tn("sl-account-modal", 0)
                            .getElementsByClassName("content")[0]
                            .children[0].children[0].removeAttribute("inert");
                    }
                }
            }
        }
    ...
    async function save() {
        ...
        if (
            id("loginname").value != get("loginname") ||
            id("loginpass").value != get("loginpass") ||
            id("loginschool").value != get("loginschool")
        ) {
            ...
            if (!n(element.getElementsByTagName("input")[1])) {
                ...
                if (!n(element.getElementsByTagName("input")[0].value)) {
                    nicknames.push([
                        ...
                        element.getElementsByTagName("input")[2].value,
        ...
        for (const element of cn("mod-slider")) {
            set(
                element.getElementsByTagName("input")[0].dataset.property,
                element.getElementsByTagName("input")[0].value +
                    element.getElementsByTagName("input")[0].dataset.unit,
            );
        }
        if (id("mod-font-file") && id("mod-font-file").files[0]) {
            ...
            id("mod-font-select") &&
            id("mod-font-select").classList.contains("mod-modified")
        ) {
            set("customfont", "");
            set("customfontname", "");
        ...
        for (const element of cn("mod-custom-setting")) {
            if (
                ...
                element.id.indexOf("bools") != -1
            ) {
                set(
                    "bools",
                    get("bools").replaceAt(
                        parseInt(
                            element.id.charAt(5) + element.id.charAt(6),
                            10,
                        ),
                        element.checked ? "1" : "0",
                    ),
                    ...
                    // File should not be compressed
                        ...
                        reader.onload = () => {
                            if (element.id == "background") {
                                set(
                                    "isbackgroundvideo",
                                    element.files[0].type.indexOf("video") !=
                                        -1,
                                );
                            }
                            set(element.id, reader.result);
        ...
        if (id("mod-bg-image").style.display == "block") {
            ...
            modMessage(
                "Bezig met opslaan",
                "Je achtergrondafbeeldingen worden nu opgeslagen. Dit kan even duren.",
                null,
                null,
                null,
                null,
            ...
            if (
                id("mod-background-wrapper").classList.contains(
                    "mod-modified",
                ) == true &&
                id("mod-background-wrapper").classList.contains(
                    "mod-loading",
        ...
        if (!n(selectedtheme)) {
            if (
                id("primarycolor").classList.contains("mod-modified") == false
            ) {
                set("primarycolor", selectedtheme.dataset.color);
            }
            if (
                id("secondarycolor").classList.contains("mod-modified") == false
            ) {
            ...
            if (selectedtheme.id != "Standaard" && selectedtheme.dataset.url) {
                toDataURL(selectedtheme.dataset.url, (dataUrl) => {
        ...
        if (cn("layout-selected", 0)) {
            set("layout", parseInt(cn("layout-selected", 0).id.charAt(7), 10));
        }
        if (id("mod-random-background")) {
            if (id("mod-random-background").classList.contains("mod-active")) {
                toDataURL("https://picsum.photos/1600/800", (dataUrl) => {
                    set("background", dataUrl);
                    filesProcessed++;
                });
            } else {
        ...
        if (id("grade-reveal-select")) {
            ...
            set(
                ...
                get("bools").replaceAt(
                    14,
                    showOnlyForNewGrades ? "1" : showAlways ? "2" : "0",
                ),
            );
        }
        ...
        modMessage(
            "Opgeslagen!",
            "Al je instellingen zijn opgeslagen.",
            "Doorgaan",
        );
    ...
    function saveReload(loadAnyway = false) {
        if (loadAnyway || filesProcessed >= cn("mod-file-input").length + 1) {
            ...
            if (get("layout") == 3) {
                tn("body", 0).style.cssText =
                    "--safe-area-inset-right: " + menuWidth + "px !important";
            } else if (get("layout") == 2 || get("layout") == 5) {
                tn("body", 0).style.cssText =
                    "--safe-area-inset-left: " + menuWidth + "px !important";
            }
        } else {
    ...
    function checkNewUser() {
        if (n(get("firstused"))) {
            ...
            if (!hasSettingsHash) {
                id("mod-welcome-open-settings").addEventListener(
                    "click",
                    () => {
                        closeWelcomeDialog();
            ...
            id("mod-welcome-close").addEventListener("click", () => {
                closeWelcomeDialog();
                if (hasSettingsHash) {
    ...
    function addSlider(name, property, min, max, unit, defaultValue) {
        ...
        if (
            get(property) == null ||
            get(property) == "" ||
            isNaN(parseInt(get(property), 10))
        ) {
            value = defaultValue + unit;
        }
        return (
            '<div class="mod-slider"><p>' +
            name +
            '</p><input id="' +
            property +
            '" title="' +
    ...
    function addSetting(
        ...
        let code =
            "<div>" +
            (n(name) ? "" : "<h3>" + name + "</h3>") +
            (n(description) || type == "checkbox"
                ? ""
                : "<div><p>" + description + "</p></div>");
        if (type == "checkbox") {
            if (key.startsWith("bools")) {
                code +=
                    '<label tabindex="0" class="switch" for="' +
                    key +
                    '"><input title="' +
                    name +
                    '" class="mod-custom-setting" type="checkbox" ' +
                    (get("bools").charAt(
                        parseInt(key.charAt(5) + key.charAt(6), 10),
                    ) == "1"
                        ? "checked"
                        : "") +
                    ' oninput="this.classList.add(\'mod-modified\');" id="' +
                    key +
            ...
            code +=
                ...
                '</div><input title="' +
                name +
                '" class="mod-custom-setting" id="' +
                key +
                '" type="range" value="' +
                (param4 != null ? value : get(key)) +
                '" min="' +
                param1 +
                '" max="' +
                param2 +
                '" step="' +
                param3 +
                "\" oninput=\"this.classList.add('mod-modified');this.parentElement.children[4].innerHTML=this.value;this.parentElement.getElementsByClassName('mod-range-preview')[0].children[0].setAttribute('style', '" +
                (param6 == "opacity"
                    ? "opacity:'+parseFloat((100 - this.value) / 100).toString()+'"
                    : "filter:" +
                      param6 +
                      "('+parseFloat(this.value / 4).toString()+'px)") +
                "');\"/><p>" +
                (param4 != null ? value : get(key)) +
                "</p><p>%</p>";
    ...
    function addTheme(name, url, primaryColor, secondaryColor) {
        ...
        if (!isNaN(parseInt(url, 10))) {
            smallimg =
                ...
                "/pexels-photo-" +
                url +
                ".jpeg?auto=compress&cs=tinysrgb&w=250";
            bigimg =
                "https://images.pexels.com/photos/" +
                url +
                "/pexels-photo-" +
    ...
    async function openModSettingsDirectly(forceOpen = false) {
        if (
            window.location.hash == "#mod-settings" ||
            get("opensettingsIntention") == "1" ||
            forceOpen
        ) {
            if (tn("sl-modal", 0)) {
                return;
    ...
    function insertModSettingLink() {
        if (
            tn("sl-account-modal", 0) &&
            tn("sl-account-modal", 0).getElementsByTagName(
                "sl-account-modal-tab",
            )[0] &&
            n(id("mod-setting-button"))
        ) {
            const modbtn = tn("sl-account-modal", 0)
                .getElementsByTagName("sl-account-modal-tab")
                [
                    tn("sl-account-modal", 0).getElementsByTagName(
                        "sl-account-modal-tab",
                    ).length - 1
            ...
            modbtn.getElementsByTagName("span")[0].innerHTML =
                "Mod-instellingen";
            modbtn.getElementsByTagName("i")[0].style.background = darkmode
                ? "#603d20"
                : "#ffefe3";
            modbtn.getElementsByTagName("i")[0].style.paddingBottom = "2px";
            modbtn.getElementsByTagName("i")[0].innerHTML = window.getIcon(
                "gear",
                null,
                "#ea9418",
            ...
            setTimeout(() => {
                ...
                if (
                    cn("data-container", 1) &&
                    cn("data-container", 1).getElementsByClassName(
                        "ng-star-inserted",
                    )[0] &&
                    n(get("realname"))
                ) {
        ...
        // Update background of modsetting link in settings menu
            ...
            id("mod-setting-button").getElementsByTagName(
                "i",
            )[0].style.background = darkmode ? "#603d20" : "#ffefe3";
        }
    }
    ...
    function pageUpdate(updateStyle = true, updateLogo = true) {
        ...
        execute([
            ...
            userName,
            teacherNicknames,
            insertModSettingLink,
            insertGradeDownloadButton,
            subjectGradesPage,
            somtodayRecap,
            rosterSimplify,
            newYearCountdown,
