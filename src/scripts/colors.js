...
function toBrightnessValue(color, target) {
    ...
    if (
        ...
        rgbcolor[2] + 50 > rgbcolor[0] + rgbcolor[1]
    ) {
        ...
        rgbcolor[0] > rgbcolor[2] &&
        rgbcolor[0] > rgbcolor[1] &&
        rgbcolor[0] + 50 > rgbcolor[1] + rgbcolor[2]
    ) {
        color = adjustColorChannel(
            2,
        ...
        rgbcolor[1] + 50 > rgbcolor[0] + rgbcolor[2]
    ) {
        color = adjustColorChannel(
            2,
            adjustColorChannel(0, color, factor),
...
function addNightTheme() {
    ...
    if (cn("toggle-systeemvoorkeur", 0)) {
        ...
        set(
            "autotheme",
            cn("toggle-systeemvoorkeur", 0).ariaChecked == "true"
                ? "true"
                : "false",
        );
        ...
        cn("toggle-systeemvoorkeur", 0).addEventListener("click", function () {
            ...
            if (this.ariaChecked == "true") {
                ...
                if (
                    ...
                    if (
                        cn("container dark", 0) &&
                        cn("container dark", 0).ariaChecked == "false"
                    ) {
                ...
                // Enable light, since this is the current preference
                    ...
                    if (
                        cn("container light", 0) &&
                        cn("container light", 0).ariaChecked == "false"
                    ) {
    ...
    const darkDiv = blokDiv?.querySelector(
        'div[data-gtm="instellingen-weergave-theme-dark-mode"]',
    );
    ...
    nightDiv.getElementsByTagName("label")[0].title =
        "Toegevoegd door Somtoday Mod";
    nightDiv.getElementsByTagName("label")[0].innerHTML =
        "Night " +
        window.logo(
            null,
            null,
            "#0099ff",
            "height:1em;width:fit-content;margin-left:5px;transform:translateY(2px);",
        );
    ...
    const allRadios = blokDiv.querySelectorAll(
        'input[type="radio"][name="thema"]',
    );
    const html = document.documentElement;
    const currentTheme = ["light", "dark", "night"].find((t) =>
        html.classList.contains(t),
    );
    if (currentTheme) {
        const activeRadio = Array.from(allRadios).find(
            (r) => r.value === currentTheme,
        );
        if (activeRadio) {
    ...
    function updateTheme() {
        ...
        if (
            checkedRadio &&
            (checkedRadio.value == "light" ||
                checkedRadio.value == "dark" ||
                checkedRadio.value == "night")
        ) {
