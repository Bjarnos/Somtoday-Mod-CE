...
async function autoLogin() {
    ...
    const schoolField = await waitForElement(
        "#organisatieSearchField, #organisatieInput",
    );
    ...
    if (id("organisatieSearchField")) {
        ...
        if (
            cn("form--checkbox checkbox-label", 0) &&
            cn("form--checkbox checkbox-label", 0).ariaChecked == "false"
        ) {
...
async function waitForPageLoad() {
    while (
        (storageMethod == "extension" || storageMethod == "indexedDB") &&
        data == null
    ) {
    ...
    // Redirect user to login page if enabled
        ...
        if (
            get("bools") == null ||
            get("bools").charAt(BOOL_INDEX.REDIRECT_ELO) == "1"
        ) {
    ...
    const loadedElement = await waitForElement(
        'sl-home > *:nth-child(2), sl-error, .errorTekst, iframe[src="https://som.today/updaten/"]',
    );
